...
class Flexi_cart extends Flexi_cart_lite
{
	...
	public function update_discount_codes($discount_codes = FALSE, $recalculate_cart = TRUE){
		//~ Modificacion que estaba metida al mirar Eneko, 2017-03-29
		if($discount_codes!=FALSE){
			//~ $articulos=$this->cart_items();
			//~ $papel=0;
			//~ foreach ($articulos as $articulo){
				//~ if($articulo["tipo"]==0)
					//~ $papel+=$articulo['quantity'];
			//~ }
			//~ if ((isset($discount_codes[0])) && $discount_codes[0]=='ENEKO'){
				...
				//~ exit;
			//~ }
			//~ while(count($discount_codes)!=0 && count($discount_codes)>$papel){
				//~ foreach($discount_codes as $d=>$v){
					//~ unset($discount_codes[$d]);
					//~ break;
				//~ }
			//~ }
			
			//~ 2017-03-29 Eneko: comentamos código anterior que solo permitía aplicar descuento si habia 'papelpintado' entre los artículos
			//~ 2017-03-29 Eneko: solo se puede aplicar un vale descuento, si hay metido uno, el nuevo no se inserta
			while(count($discount_codes)>1){
				foreach($discount_codes as $d=>$v){
					unset($discount_codes[$d]);
					break;
				}
