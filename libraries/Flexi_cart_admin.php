...
class Flexi_cart_admin extends Flexi_cart_lite
{
	...
	public function get_db_discount_query($sql_select = FALSE, $sql_where = FALSE)
	{
		if ($this->CI->flexi_cart_admin_model->get_enabled_status('discounts'))
		{
			...
			$this->CI->db->order_by('disc_description','ASC');
			//~ $this->sql_order_by($order_by);
	...
	public function insert_db_discount_generica($sql_insert, $tabla_descuentos){
		// If a discount code is set, check it is unique.
		if (isset($sql_insert[$this->CI->flexi->cart_database[$tabla_descuentos]['columns']['code']]) && ! empty($sql_insert[$this->CI->flexi->cart_database[$tabla_descuentos]['columns']['code']]))
		{
			if (! $this->CI->flexi_cart_admin_model->check_code_available($sql_insert[$this->CI->flexi->cart_database[$tabla_descuentos]['columns']['code']]))
			{
				$this->CI->flexi_cart_admin_model->set_error_message('duplicate_discount_code', 'config');
				return FALSE;
			}
		...
		return $this->CI->flexi_cart_admin_model->insert_table_data($this->CI->flexi->cart_database[$tabla_descuentos]['table'], $sql_insert, 'discounts');
	}
	...
	public function update_db_discount_generica($sql_update, $sql_where, $tabla_descuentos){
		if ($this->CI->flexi_cart_admin_model->get_enabled_status('discounts')){
			// If a discount code is set, check it is unique.
			if (isset($sql_update[$this->CI->flexi->cart_database[$tabla_descuentos]['columns']['code']]) && ! empty($sql_update[$this->CI->flexi->cart_database[$tabla_descuentos]['columns']['code']]))
			{
				$table_row_id = (is_array($sql_where)) ? $sql_where[$this->CI->flexi->cart_database[$tabla_descuentos]['columns']['id']] : $sql_where;
				$code = $sql_update[$this->CI->flexi->cart_database[$tabla_descuentos]['columns']['code']];
			...
			$default_pk = $this->CI->flexi->cart_database[$tabla_descuentos]['columns']['id'];

			// Prevent reward voucher data being updated.
			$this->CI->db->where($this->CI->flexi->cart_database[$tabla_descuentos]['columns']['type'].' != 3');
			
			return $this->CI->flexi_cart_admin_model->update_table_data($this->CI->flexi->cart_database[$tabla_descuentos]['table'], $sql_update, $sql_where, $default_pk, 'discounts');
		}
	...
	public function delete_db_discount_generica($sql_where, $tabla_descuentos){
		if ($this->CI->flexi_cart_admin_model->get_enabled_status('discounts')){
			$default_pk = $this->CI->flexi->cart_database[$tabla_descuentos]['columns']['id'];

			$this->CI->db->where($this->CI->flexi->cart_database[$tabla_descuentos]['columns']['type'].' != 3');
			
			return $this->CI->flexi_cart_admin_model->delete_table_data($this->CI->flexi->cart_database[$tabla_descuentos]['table'], $sql_where, $default_pk, 'discounts');
		}
		
		return FALSE;
	}
	
	public function get_db_discount_generic_array($sql_where = FALSE, $table='discounts', $sql_select=FALSE){
		if ($this->CI->flexi_cart_admin_model->get_enabled_status($table)){
			$default_pk = $this->CI->flexi->cart_database[$table]['columns']['id'];

			$this->CI->db->where($this->CI->flexi->cart_database[$table]['columns']['type'].' != 3');
			$this->CI->db->order_by('disc_description','ASC');
			
			return $this->CI->flexi_cart_admin_model
				->get_table_data($this->CI->flexi->cart_database[$table]['table'], $sql_select, $sql_where, $default_pk, $table)->result_array();
		}
		return FALSE;
	}
