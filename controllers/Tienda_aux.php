...
class Tienda extends CI_Controller {
    ...
    function index() {
        ...
        $this->data['mosaico'] = $this->contenido_model->get_imagenes_front_zona("mosaico");
    ...
    function busqueda() {
        if (isset($_GET['ajax']) && isset($_GET['q'])) {
            ...
            echo json_encode(strlen($q) >= 3 ? $this->flexi_cart_model->search_items_autocomplete($q) : array());
            return;
        }
        // Admite búsqueda por GET (?q=) además de POST (para back button y "Ver todos")
        $search_get = isset($_GET['q']) ? trim($_GET['q']) : '';
        if ($search_get != '') {
            $_POST['search'] = $search_get;
        }
        if ((isset($_POST['search']) && trim($_POST['search']) != "") || $search_get != '') {
            $search = $search_get != '' ? $search_get : trim($_POST['search']);
            /*
            print '<pre><xmp>';
            print_r($_POST);
            print '</xmp></pre>';
    ...
    function listado_categorias_nuevo($tipo_producto=0, $idnueva_categoria_familia=0){
        ...
        switch ($this->data['categ']) {
            ...
            case 1: 
                ...
                $categ = "Fotomurales";
                $categ_txt_desc = "Todos los fotomurales agrupados";
                break;
            case 2: 
    ...
    function listado_nueva_categoria($categoria_seo){
        ...
        $this->data['filtros_categorias_seo']= $this->flexi_cart_model->get_filtros_izquierda_listado( $categoria_seo->tipo_producto, $categoria_seo->nueva_categoria_id);
        /*
        ...
        */
        $this->db->cache_on();
        $this->data['all'] = $this->flexi_cart_model->get_items_filtros_nuevo_listado($categoria_seo->tipo_producto, $this->data['filtros_categorias_seo']['ids_categoria'], 0, $ord, $categoria_seo->nueva_categoria_id);
        $this->db->cache_off();
    ...
    function listado_productos_nuevo($categoria_principal, $param1='', $param2='', $param3='', $param4='', $param5=''){
        ...
        switch ($categoria_principal) {
            ...
            case 'fotomurales': 
                ...
                $categoria_principal='tienda/fotomurales';
                $this->data['categ']=1;
                $this->data['meta_title']='Fotomurales - Personaliza tu Espacio'; 
                $this->data['meta_description']='Elige tu Fotomural y consigue el efecto deseado en tu estancia. En nuestro amplio catálogo encontrarás lo que buscas. ¡Visítanos!'; 
        ...
        */
        ...
        $this->data['all'] = $this->flexi_cart_model->get_items_filtros_nuevo_listado($this->data['categ'], $this->data['filtros_categorias_seo']['ids_categoria'], 0, $ord);
        /*
        print '<pre><xmp>';
            ...
            foreach($a_categorias_productos as $tipo_aux){
                switch ($tipo_aux) {
                  case 0: $categorias_txt[] = "Papel Pintado";
                      break;
                  case 1: $categorias_txt[] = "Fotomurales";
                      break;
    ...
    function listado_marcas($tipo_producto=0, $id_marca=''){
        ...
        switch ($this->data['categ']) {
            ...
            case 1: 
                ...
                $categoria_principal='/tienda/fotomurales';
                $url_categoria_principal='fotomurales';
                $this->data['meta_title'] = "Listado de marcas de Fotomurales. ¡Personaliza tu Espacio!"; 
                $this->data['meta_description'] ='Todos los fotomurales agrupados por marcas, seguro que tenemos lo que buscas. ¡Visítanos!'; 
                break;
            case 2: 
                $categ = "Revestimientos";
                $categoria_principal='/tienda/revestimientos';
        ...
        $this->data['familia_producto'] = $categ;
        if ($tipo_producto!=-1){
            // 2025-08-05 Para papeles pintados vamos a mostrar todas las marcas, no solo las de papeles
            if ($tipo_producto==0){
                ...
                $this->data['a_migas'][$categoria_principal]=$categ;
                $this->data['fab'] = $this->flexi_cart_model->get_categories('all');
            }
            else{
                $this->data['url_categoria_principal'] = $url_categoria_principal;
                $this->data['url_marca'] = $categoria_principal.'/marca';
        ...
        if($categ=='Foto Murales') // lo cambiamos para el encabezado, para buscar las marcas foto murales debe ir separado
            $categ='Fotomurales';
       // $this->data['col'] = $this->flexi_cart_model->get_col_array();
        ...
        if ($id_marca!=''){
            ...
            if (is_object($marca)){
                ...
                if($url_categoria_principal!='marca'){
                    $categorias_productos=str_replace('Foto Murales', 'Fotomurales',$marca->cats);
    ...
    private function orden($orden) {
        ...
        if ($orden == "4"){
            return '';
            //return 'portada desc, demo_items.item_id desc';
        }
        return '';
    }
    ...
    function fotomurales($param1 = "", $param2 = "", $param3 = "", $param4 = "", $param5 = "") {
        if ($param1=='marcas' || $param1=='marca'){
            ...
            if ((int)$id_coleccion!=0)
                $this->listado_productos_coleccion('fotomurales', $id_coleccion);
            elseif ((int)$id_marca!=0)
            ...
            $this->listado_productos_nuevo('fotomurales');
    }
