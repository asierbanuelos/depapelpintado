      ...
      foreach ($registros as $key => $item) {
        if(strlen($item->meta_titlec)>100 || trim($item->meta_titlec)==''){
          ...
          if (count($categorias)>1){
            ...
            $item->meta_description='Colección '.$item->coleccion_name.' de '.implode(', ', $categorias).' y '.$ultimo_elemento.' creada por '.$item->cat_name.'. Catálogo actualizados en nuestra web. ¡Descúbrelo!';
          }
          else{
