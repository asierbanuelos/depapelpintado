...
class Tienda extends CI_Controller {
    ...
    function __construct() {
		...
		if ($this->db->where(array('email' => $this->input->post("email")))->count_all_results('users') == 0)
			$this->data["logmsg"] = 'No existe ninguna cuenta asociada a ese email. Regístrate com nuevo usuario.';
		else{
    ...
    function index() {
        ...
        $this->data['description'] = "dePapelPintado, tu tienda de confianza. Especialistas en papel pintado, amplio catálogo de marcas y colecciones con las últimas novedades. Más de 40 años de experiencia con la garantía de Ekam Decoración. ¡Descúbrenos!";
        ...
        $this->data['url_canonica'] = base_url();
        if (isset($_GET['test']) && $_GET['test']='eneko'){
            $this->load->view('frontend/header_menu', $this->data);
        }
        else
            $this->load->view('frontend/header', $this->data);
    ...
    function index_eneko() {
        ...
        $this->load->view('frontend/new_header', $this->data);
        ...
        if (isset($_POST['search']) && trim($_POST['search']) != "") {
            ...
            //$this->load->view('frontend/slider', $this->data);
            $this->load->view('frontend/slider_nuevo', $this->data);
            $this->load->view('frontend/new_home', $this->data);
        }
        //    $this->load->view('frontend/estilos', $this->data);
        $this->load->view('frontend/new_footer', $this->data);
        //RESTUARAR
        //$this->view_cart();
        //redirect('admin_library/articulos');
    }
    ...
    function listado_marcas($tipo_producto=0, $id_marca=''){
        ...
        switch ($this->data['categ']) {
            case 0: 
                ...
                $this->data['description2'] ='Todos papeles pintados agrupados por marcas, seguro que tenemos lo que buscas. ¡Visítanos!'; //title2 para evitar que añada depapelpintado al final
                break;
            case 1: 
                $categ = "Fotomurales";
                break;
        ...
        $this->data['a_migas'][$url_categoria_principal]=$categ;
        ...
        if ($id_marca!=''){
            ...
            if (is_object($marca)){
                $this->data['fab'] = $marca;
                $this->data['col'] = $this->flexi_cart_model->get_col($id_marca, $tipo_producto);

                $this->data['a_migas']['marcas-'.$url_categoria_principal.'/'.$this->urlenc_aux($marca->cat_name).'/'.$id_marca]=$marca->cat_name;
        ...
        if (isset($coleccion[0])){
            ...
            switch ($categoria_principal) {
                case 'papel-pintado': 
                    $categ = "Papel Pintado";
                    $this->data['categ']=0;
                    $this->data['title2']='Colección '.$coleccion[0]->coleccion_name.' de Papel Pintado '.$marca->cat_name; 
                    ...
                    $this->data['texto_h1_seccion']='Colección '.$coleccion[0]->coleccion_name.' de Papel Pintado '.$marca->cat_name; 
    ...
    function listado_categorias_nuevo($tipo_producto=0, $idnueva_categoria_familia=0){
        ...
        switch ($this->data['categ']) {
            case 0: 
                ...
                break;
            case 1: 
                $url_categoria_principal='fotomurales';
                $url_tienda='tienda/fotomurales';
                $categ = "Fotomurales";
                break;
            case 2: 
        ...
        switch ($idnueva_categoria_familia) {
            case 1:
                ...
                $this->data['title2'] ='Estilos de papel pintado'; //title2 para evitar que añada depapelpintado al final
                $this->data['description2'] ='Todos papeles pintados agrupados por estilos, seguro que tenemos lo que buscas. ¡Visítanos!'; //title2 para evitar que añada depapelpintado al final
        ...
        $this->load->view('frontend/header', $this->data);
        if (isset($_GET['test']))
            $this->load->view('frontend/listado-categorias-eneko', $this->data);
        else    
    ...
    function listado_nueva_categoria($categoria_seo){
        // Función al entrar en una categoría SEO concreta
        ...
        */
        ...
        if (isset($_POST['search']) && trim($_POST['search']) != "") {
            ...
            */
            ...
            //$this->data['menu_categorias_seo']=$this->flexi_cart_model->get_categorias_seo_menu($categoria_seo->tipo_producto);
    ...
    function listado_productos_nuevo($categoria_principal, $param1='', $param2='', $param3='', $param4='', $param5=''){
        ...
        switch ($categoria_principal) {
            case 'papel-pintado': 
                ...
                $categoria_principal='tienda/papel_pintado';
                $this->data['categ']=0;
                $this->data['title2']='Papel pintado para Pared - Amplio Catálogo'; 
                $this->data['description2']='Elige el Papel Pintado que mejor se adapte a tu ambiente. En nuestro amplio catálogo con las últimas novedades y más de 1000 muestrarios encontrarás lo que buscas. ¡Visítanos!'; 
                $this->data['texto_h1_seccion']='Papeles Pintados'; 
                $this->data['texto__intro_seo']= $this->load->view('textos_seo/papel-pintado-seo', $this->data, true); 
                $this->data['alt_slider']='papel-pintado';
                $this->data['images'] = $this->contenido_model->get_imagenes(true, "papelpintado");
    ...
    function papel_pintado($param1 = "") {
        // Redirecciones para pasar a las nuevas urls
        /*
        if (isset($_POST['search']) && trim($_POST['search']) != "") {
        } 
        elseif ($param1=='marca'){
            if ($this->uri->segment(6)) {
                ...
                header("Location: https://www.depapelpintado.es/marcas-papel-pintado/".$this->uri->segment(5)."/".$this->uri->segment(7)."/".$this->uri->segment(6)); 
                header("Connection: close");        
                exit;     
            }
            else{
                header("HTTP/1.1 301 Moved Permanently"); 
                header("Location: https://www.depapelpintado.es/marcas-papel-pintado/".$this->uri->segment(5)."/".$this->uri->segment(4)); 
                header("Connection: close");        
                exit;     
            }
        }
        elseif ($param1=='marcas'){
            header("HTTP/1.1 301 Moved Permanently"); 
            header("Location: https://www.depapelpintado.es/marcas-papel-pintado"); 
            header("Connection: close");        
            exit;     
        }
        else{
            if($this->uri->segment(3) == "economicos"){
                ...
                $es_color=false;
                $es_estilo=false;
                foreach($this->uri->segments as $txt_segmento){
                    if ($txt_segmento=='color'){
                        $es_color=true;
                        $es_estilo=false;
                    }
                    elseif ($txt_segmento=='estilo'){
                        $es_color=false;
                        $es_estilo=true;
                    }
                    elseif ($txt_segmento=='orden'){
                        $es_color=false;
                        $es_estilo=false;
                    }

                    if ($es_color && $txt_segmento!='color')
                        $a_colores[]=$txt_segmento;
                    if ($es_estilo && $txt_segmento!='estilo')
                        $a_estilos[]=$txt_segmento;
                ...
                $parametros=array();
                $id_colores=array();
                foreach ($a_colores as $colot_txt){
                    $result = $this->db->from("demo_gama")->where(array('gama_name' => str_replace("-", " ", $colot_txt)))->get()->row();
                    if (isset($result->gama_id))
                        $id_colores[]=$result->gama_id;
                }
                if (count($id_colores))
                    $parametros[]='color='.implode(',', $id_colores);
                
                $id_estilos=array();
                foreach ($a_estilos as $estilo_txt){
                    $result= $this->db->from("demo_estilo")->where(array('estilo_name' => str_replace("-", " ", rawurldecode(strtoupper($estilo_txt))), 'activo' => 1))->get()->row();
                    if (isset($result->estilo_id))
                        $id_estilos[]=$result->estilo_id;
                }
                if (count($id_estilos))
                    $parametros[]='estilo='.implode(',', $id_estilos);
            
                //$aux = $this->uri->uri_to_assoc(3);
                //if(isset($aux['color'])){
                //    $idcolor = $this->db->from("demo_gama")->where(array('gama_name' => str_replace("-", " ", $aux['color'])))->get()->row()->gama_id;
                //    $parametros[]='color='.$idcolor;
                //}
                //if(isset($aux['estilo'])){
                //    $idestilo= $this->db->from("demo_estilo")->where(array('estilo_name' => str_replace("-", " ", rawurldecode(strtoupper($aux['estilo']))), 'activo' => 1))->get()->row()->estilo_id;
                //    $parametros[]='estilo='.$idestilo;
                //}
                $parametros_txt='';
                if(count($parametros)){
                    $parametros_txt='?'.implode('&', $parametros);
                }
                header("HTTP/1.1 301 Moved Permanently"); 
                header("Location: https://www.depapelpintado.es/papel-pintado".$parametros_txt); 
                header("Connection: close");        
                exit;     
            }
        }
        */
        ...
        $description = "Elige el Papel Pintado que mejor se adapte a tu ambiente. En nuestro amplio catálogo con las últimas novedades y más de 1000 muestrarios encontrarás lo que buscas. ¡Visítanos!";
        ...
        if (isset($this->data['donde']['estilo'])) {
            ...
            //$this->data['datos_estilo']=$this->flexi_cart_model->get_estilo($this->data['donden']['estilo']);
            //$estilo = " - " . $this->data['donde']['estilo'];
            $row_estilo = $this->data['donden']['estilo'] = $this->db->from("demo_estilo")->where(array('estilo_name' => str_replace("-", " ", rawurldecode(strtoupper($this->data['donde']['estilo']))), 'activo' => 1))->like("cats", "Papel Pintado", "both")->get()->row();

            if (isset($row_estilo->estilo_id)){
                $this->data['donden']['estilo'] =$row_estilo->estilo_id;
                $this->data['datos_estilo']=$this->flexi_cart_model->get_estilo($this->data['donden']['estilo']);
        ...
        if (isset($this->data['donde']['color'])) {
            //$this->data['donden']['color'] = $this->db->from("demo_gama")->where(array('gama_name' => str_replace("-", " ", $this->data['donde']['color'])))->get()->row()->gama_id;
            //$color = " - " . $this->data['donde']['color'];
            $row_color = $this->db->from("demo_gama")->where(array('gama_name' => str_replace("-", " ", $this->data['donde']['color'])))->get()->row();
            if (isset($row_color->gama_id)){
                $this->data['donden']['color'] = $row_color->gama_id;
                $color = " - " . $this->data['donde']['color'];
            }
            else
                $this->data['donden']['color'] = null;
        ...
        $this->data['title'] = $title . $estilo . $color." - " ;
        $this->data['familia_producto'] = $categ;
        ...
        if (isset($_POST['search']) && trim($_POST['search']) != "") {
            ...
            ///////////////////
                ...
                */
                ...
                $this->data['categoria_principal'] = 'tienda/papel_pintado';
                $this->data['a_migas']['/tienda/papel_pintado']=$categ;

                if (isset($_GET['test']) && $_GET['test']=='eneko')
                    $this->load->view('frontend/cuerposeccion-con-filtros-eneko', $this->data);
                else
                    $this->load->view('frontend/cuerposeccion-con-filtros', $this->data);
                /*
                if (isset($_GET['test']) && $_GET['test']=='eneko'){
            ...
            */
                ...
                $this->data['texto_h1_seccion']='Papel Pintado Outlet'; 
    ...
    function fotomurales($param1 = "") {
        ...
        if (isset($this->data['donde']['estilo'])) {
            $row_estilo = $this->db->from("demo_estilo")->where(array('estilo_name' => str_replace("-", " ", $this->data['donde']['estilo']), 'activo' => 1))->like("cats", "Foto Murales", "both")->get()->row();
            if (isset($row_estilo->estilo_id)){
                $this->data['donden']['estilo'] =$row_estilo->estilo_id;
                $estilo = " - " . $this->data['donde']['estilo'];
                $this->data['datos_estilo']=$this->flexi_cart_model->get_estilo($this->data['donden']['estilo']);
        ...
        if ($param1 == "marcas"){
            $title.=" - Listado de Marcas";
            $this->data['title2'] = "Listado de marcas de Fotomurales. ¡Personaliza tu Espacio!"; // en title2 damos prioridad a ese título
        }
        else if (isset($this->data['donde']['marca'])) {
            if (isset($this->data['fab']) && $this->data['fab'] != null) {
                $f = $this->flexi_cart_model->get_fab($this->uri->segment(4));
        ...
        if (isset($_POST['search']) && trim($_POST['search']) != "") {
            ...
            if ($this->uri->segment(6)) {
                ...
                $this->data['all'] = $this->flexi_cart_model->get_items_cole($this->uri->segment(6), false, $this->data['categ']);
                $this->db->cache_off();
                if ($this->uri->segment(6)==1766){
                    /*
                    echo "<br />dentro de 1766";
                    $miprueba=$this->flexi_cart_model->get_items_cole($this->uri->segment(6), false, $this->data['categ']);
                    echo "<br />".$this->db->last_query();
                    print '<pre><xmp>';
                    print_r($miprueba);
                    print '</xmp></pre>';
                    print '<pre><xmp>';
                    print_r($this->data['datos_coleccion']);
                    print '</xmp></pre>';
                    exit;
                    */
                }
            ...
            $this->data['texto_h1_seccion']='Fotomurales'; 
            ...
            */
            ...
            $this->data['categoria_principal'] = 'tienda/fotomurales';
            $this->data['a_migas']['/tienda/fotomurales']=$categ;
...
// introducir aqui el contenido a mostrar en los metas
//  para el titulo sigue este formato: "Papelpintado %title% - %color estilo y demas%"
        $title = "Revestimientos de Pared";
        $description = "Elige el Revestimiento adecuado para tu espacio y decora tu pared. En nuestro amplio catálogo encontrarás lo que buscas. ¡Visítanos!";
...
// NO TOCAR A PARTIR DE AQUI      
        ...
        if (isset($this->data['donde']['estilo'])) {
           // $this->data['donden']['estilo'] = $this->db->from("demo_estilo")->where(array('estilo_name' => str_replace("-", " ", $this->data['donde']['estilo']), 'activo' => 1))->like("cats", "Revestimientos", "both")->get()->row()->estilo_id;
            ...
            $row_estilo = $this->db->from("demo_estilo")->where(array('estilo_name' => str_replace("-", " ", $this->data['donde']['estilo']), 'activo' => 1))->like("cats", "Revestimientos", "both")->get()->row();
            if (isset($row_estilo->estilo_id)){
                $this->data['donden']['estilo'] =$row_estilo->estilo_id;
                $estilo = " - " . $this->data['donde']['estilo'];
                $this->data['datos_estilo']=$this->flexi_cart_model->get_estilo($this->data['donden']['estilo']);
...
$this->db->cache_off();
        ...
        if ($param1 == "marcas")
            ...
            if (count($this->data['donde']) == 0){
                //$this->load->view('frontend/slider2', $this->data);
                $this->data['alt_slider']='revestimientos';
                $this->load->view('frontend/slider_nuevo', $this->data);
            }
                //$this->data['menu_categorias_seo']=$this->flexi_cart_model->get_categorias_seo_menu_sin_estilos($this->data['categ']);
                $this->data['menu_categorias_seo']=$this->flexi_cart_model->get_categorias_seo_menu($this->data['categ']);
            ...
            */
            ...
            $this->data['categoria_principal'] = 'tienda/revestimientos';
            $this->data['a_migas']['/tienda/revestimientos']=$categ;
...
// NO TOCAR A PARTIR DE AQUI      
        ...
        if (isset($this->data['donde']['estilo'])) {
            ...
            //$estilo = " - " . $this->data['donde']['estilo'];
            $row_estilo = $this->db->from("demo_estilo")->where(array('estilo_name' => str_replace("-", " ", $this->data['donde']['estilo']), 'activo' => 1))->like("cats", "Telas", "both")->get()->row();
            if (isset($row_estilo->estilo_id)){
                $this->data['donden']['estilo'] =$row_estilo->estilo_id;
                $estilo = " - " . $this->data['donde']['estilo'];
                $this->data['datos_estilo']=$this->flexi_cart_model->get_estilo($this->data['donden']['estilo']);
...
$this->db->cache_off();
        ...
        if ($param1 == "marcas")
            ...
            */
            ...
            $this->data['categoria_principal'] = 'tienda/telas';
            $this->data['a_migas']['/tienda/telas']=$categ;
...
// NO TOCAR A PARTIR DE AQUI      
        ...
        if (isset($this->data['donde']['estilo'])) {
            ...
            //$estilo = " - " . $this->data['donde']['estilo'];
            $row_estilo = $this->db->from("demo_estilo")->where(array('estilo_name' => str_replace("-", " ", $this->data['donde']['estilo']), 'activo' => 1))->like("cats", "Alfombras", "both")->get()->row();
            if (isset($row_estilo->estilo_id)){
                $this->data['donden']['estilo'] =$row_estilo->estilo_id;
                $estilo = " - " . $this->data['donde']['estilo'];
                $this->data['datos_estilo']=$this->flexi_cart_model->get_estilo($this->data['donden']['estilo']);
        ...
        if ($param1 == "marcas")
            ...
            if (count($this->data['donde']) == 0){
                //$this->load->view('frontend/slider2', $this->data);
                $this->data['alt_slider']='alfombras';
                $this->load->view('frontend/slider_nuevo', $this->data);
            }
            ...
            $this->data['categoria_principal'] = 'tienda/alfombras';
            $this->data['a_migas']['/tienda/alfombras']=$categ;
...
// NO TOCAR A PARTIR DE AQUI      
        ...
        $this->data['title2'] = '¿Qué tipos de herramientas necesito para empapelar?';
        $this->data['description2'] = 'Si estás pensando en empapelar una habitación de tu casa, es importante que sepas cuáles son las herramientas que necesitas para hacerlo correctamente. Rodillo o pincel de cola, tijeras de papel tapiz, brocha de empapelar... ¡Tenemos todo lo que necesitas!'; 
        $this->load->view('frontend/header', $this->data);

        if (count($this->data['donde']) == 0){
            //$this->load->view('frontend/slider2', $this->data);
            $this->data['alt_slider']='herramientas-para-empapelar';
            $this->load->view('frontend/slider_nuevo', $this->data);
        }
...
//  para el titulo sigue este formato: "Papelpintado %title% - %color estilo y demas%"
        $title = "";
        $description = "Catálogos actualizados de las mejores marcas de papel pintado, fotomurales, revestimientos, telas y alfombras. ¡Descúbrelos!";
        $keywords = "";
// NO TOCAR A PARTIR DE AQUI      
        $estilo = "";
        $color = "";
        ...
        if (!isset($this->data['donde']['marca'])){
            //Todas las marcas
            //$title.=" - Listado de Marcas";
            $this->data['title2'] = "Todas las marcas | dePapelPintado.es"; // en title2 damos prioridad a ese título
        }
        else if (isset($this->data['donde']['marca'])) {
            if (isset($this->data['fab']) && $this->data['fab'] != null) {
                $title=ucwords(str_replace('-', ' ', str_replace('-and-', ' & ', rawurldecode($this->uri->segment(5)))));
        ...
        if (empty($this->data['key'])){
            ...
            switch ($this->data['key']['item_tipo']) {
                case 0: $categ = "Papel Pintado";$tipo_producto = "Papel Pintado";
                    break;
                case 1: $categ = "Fotomurales";$tipo_producto = "Fotomural";
                    break;
            ...
            if (isset($_GET['test'])){
                ...
                print '</xmp></pre>';
                print '<pre><xmp>';
                print_r($this->data['otro']);
                print '</xmp></pre>';
                if ($this->data['key']['item_id']==80993){
                }
                */
                $this->load->view('frontend/articulo_grecas', $this->data);
            }
            else
    ...
    function genera_feed_google_merchant() {
        $marcas=$this->flexi_cart_model->get_erregistroak_eremu_guztiak('demo_categories', 'cat_id', array('publico'=>1));
        $a_marcas=array();
        foreach($marcas as $id_marca =>$datos){
            if ($datos['activo']==1 && trim($datos['cat_name'])!='')
    ...
    private function item_feed_google_merchant($row, $a_colecciones, $descuento_producto) {
        ...
        if (trim($row['item_name'])!='')
            $item.= "<g:title>".$row["item_name"]."</g:title>\n";
        else{
        ...
        $item.= "<g:description>".$row["meta_description"]."</g:description>\n";
        ...
        $item.= "<g:brand>".utf8_encode($a_colecciones[$row['item_coleccion_id']]['marca_name'])."</g:brand>\n";
    ...
    function sitemap_marcas_colecciones() {
        ...
        echo "</url>\n";
        echo "<url>\n";
        echo "       <loc>https://www.depapelpintado.es/marcas-papel-pintado</loc>\n";
        echo "</url>\n";
        echo "<url>\n";
    ...
    private function sitemap_categoria($categoria) {
        ...
        if ($categoria=='Foto Murales')
            $categoria='Fotomurales';
        ...
        if ($categoria=='Papel_pintado'){
            /*
            echo "<url>\n";
            echo "       <loc>https://www.depapelpintado.es/".$this->urlenc_aux($categoria_original)."</loc>\n";
            echo "</url>\n";
            */
            echo "<url>\n";
...
$this->db->cache_off();
    ...
    function get_next_categoria_seo($fabs = 0, $est = 0, $gamas = 0, $page = -1, $ord = '') {
        ...
        if (isset($_POST['search']) && trim($_POST['search']) != "") {
            ...
            $this->data['all'] = $this->flexi_cart_model->get_items_categoria_seo($categoria_seo[0], $fab, $estilo, $color, $page, $ord);
            //$this->data['all'] = $this->flexi_cart_model->get_items_categoria_seo($categoria_seo, null, null, $page, $categ, $econ, $losmas, $ord);
    ...
    function get_next_listado_filtros($page = -1) {
        ...
        */
        ...
        $this->data['filtros_categorias_seo']= $this->flexi_cart_model->get_filtros_nuevo_listado($tipo_producto);
        $this->data['all'] = $this->flexi_cart_model->get_items_nuevo_listado_filtros($tipo_producto, $this->data['filtros_categorias_seo']['ids_categoria'], $page, $ord);
