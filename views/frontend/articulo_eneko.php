...
<div class="units-row ficha end">
  <div class="unit-100">
    ...
    if($key['item_tipo']!=5){
      ...
      ?>
       <h1 class="fuentecorporativa magenta-secundario <?php echo $nomargin; ?>"><? echo $tipo_producto.' '.$key['cat_name']." ".$key['coleccion_name']." ".$key['item_ref']?></h1>
      <?
      }
    else{
      $nombre_completo="Herramientas ".$key['item_name'];
      ?>
       <h1 class="fuentecorporativa magenta-secundario <?php echo $nomargin; ?>"><?echo $key['item_name']?></h1>
      <?
    }
    ?>
  </div>
...
<div class="units-row ficha end">
  ...
  if($categ!=5){
    ...
    // Esto solo para herramientas
    ?>
    <div class="unit-50">
      <?php echo $key['item_text'];?>
      <p class="unidades">
...
<?php 
if ($key['item_tipo']==5){ // HERRAMIENTA?>
...
// A partir de aquí solo si no son herramintas, ya que el formulario de comra etc los hemos tratado más arriba 
if($key['item_tipo']!=5){
  if (trim($key['col_text'])!='' && $key['col_text_publico'] ){
    ?>
    <!--  <h3> Descripción del producto </h3>-->
    <div class="unit-100">
      <div class="contenido-colapsable contenido-colapsable-oculto">
        <h2>Colección <?php echo $key['coleccion_name']; ?> de <?php echo $key['cat_name']; ?></h2>
        <?php echo $key['col_text'];?>
      </div>
      <button class="my_collapsible my_collapsible_coleccion" aria-label="Mostrar información de la colección"></button>
    </div>
    <?php 
  }
  else{
    ?>
    <p><?php echo $key['col_text'];?></p>
    <?php
  }
  ?>
  ...
  <div class="units-row end">
    ...
    <div class="unit-50">
      ...
      if(($categ==0 || $categ==2) && $key['item_unidad']!="Unidad" ){?>
        ...
        <p>
          <input type="text" class="calcuinput" id="paredancho" value="" placeholder="Ancho Pared en metros"/>
      ...
      <p class="precio magenta-secundario">
        <b>Precio: </b>
        <?php
        $precio=$key['item_price'];
        // print_r($key);
  ...
  <div class="units-row end" id='articulos-relacionados'>
    <div class="unit-100">
      <?php $this->load->view('frontend/otrosarticulos_nuevo'); ?>
    </div>
  </div>
...
document.getElementById('inicio').scrollIntoView();
 ...
 Shadowbox.init({
  ...
  $('.maincont').zoomy({innerZoom:true,magnify:1.2, lensHeight: 285,lensWidth: 285});
