...
*/
	...
	public function validate_discount_codes($discount_codes)
	{
		...
		$sql_where = $tbl_cols_discount['usage_limit']." > 0 AND
            ".$tbl_cols_discount['valid_date']." <= '".$this->database_date_time()."' AND 
	...
	public function required_columns()
	{
        ...
        function get_item($id=0,$cat=0){
          $temp = $this->db->select("*".($cat!=5)?",GROUP_CONCAT(gama_name SEPARATOR ', ') AS color":"",FALSE)
			->from('demo_items')
    ...
    function get_items_cole ($cole=""){
      ...
      $temp->from('demo_items')
            ...
            $temp->join('(discount_group_items JOIN discounts ON disc_group_item_group_fk = disc_group_fk) ', 'disc_group_item_item_fk = item_id','left');
             ...
             return $temp->group_by('item_id')->order_by('item_id')->get()->result_array();
    }
    function search_items($search="",$page=-1){
     ...
     if(count($exploded)>0){
      ...
      if(count($estiloget)>0){
        ...
        $s.=" OR `estilo_item_item` IN (".implode(",", $e).")";
      }
      if(count($colecget)>0){
        $c=array();
      ...
      $temp->where("(".$s.")");
      if($page>0)$temp->limit(30,$page*30);
      else $temp->limit(30,0);
      return $temp->group_by('item_id')->order_by('orden')->get()->result_array();
    }
    
    function get_items_filter($fab=null,$estilo=null,$gama=null,$page=-1, $categ=0, $econ=0,  $order='orden'){
       $temp=$this->db->select("*",FALSE);
            $temp->from('demo_items');
            //->join($this->flexi_cart->db_table('item_stock'), 'item_id = '.$this->flexi_cart->db_column('item_stock', 'item'));
           if($categ!=5){
    ...
    function get_fab($fab=null){
      if ($fab!=null){
        ...
        $temp->from('demo_categories');
           return $temp->where('cat_id',$fab)->get()->row();
      }
    }
      ...
      if ($fab!=null){
        ...
        $whereclause = "(ccats LIKE '%".$categ."%') ";
         $temp->where($whereclause);
           return $temp->where('coleccion_cat_id',$fab)->where('activo',1)->get()->result_array();
      }
    }
    private function get_item_data($item = FALSE, $item_data = FALSE, $insert = FALSE, $force_column_update = FALSE)
	{
    ...
    function get_categories($cats="",$order="ASC"){
      ...
      if($cats=="")return $this->db/*->select("cat_name AS n, cat_id AS i,",FALSE)*/->from('demo_categories')->where($whereclause)->where(array('activo'=>1))->order_by('cat_name',$order)->get()->result();
      else return $this->db/*->select("cat_name AS n, cat_id AS i,",FALSE)*/->from('demo_categories')->like('cats',$cats,'both')->where(array('activo'=>1))->order_by('cat_name',$order)->get()->result();
      
    }
    function get_categoriessample($cats=""){
      $whereclause = "(cats LIKE '%Papel Pintado%' OR cats LIKE '%Foto Murales%') ";
      if($cats=="")return $this->db/*->select("cat_name AS n, cat_id AS i,",FALSE)*/->from('demo_categories')->where($whereclause)->where(array('activo'=>1))->limit(16)->order_by('cat_name','RAND')->get()->result();
      else return $this->db/*->select("cat_name AS n, cat_id AS i,",FALSE)*/->from('demo_categories')->like('cats',$cats,'both')->where(array('activo'=>1))->limit(16)->order_by('cat_name','RAND')->get()->result();
    ...
    function get_colecciones($cat=0,$order="ASC"){
      if ($cat==0 )return $this->db->from('demo_coleccion')->join('demo_categories', 'coleccion_cat_id = cat_id')->where(array('demo_coleccion.activo'=>1))->order_by('coleccion_id',$order)->get()->result();
      else return $this->db->from('demo_coleccion')->join('demo_categories', 'coleccion_cat_id = cat_id')->where(array('coleccion_cat_id'=>$cat,'demo_coleccion.activo'=>1))->order_by('coleccion_id',$order)->get()->result();
    }
    ...
    function get_gamas($order="ASC"){
      return $this->db->from('demo_gama')->where(array('activo'=>1))/*->order_by('gama_principal','desc')*/->order_by('gama_name',$order)->get()->result();
    }
    ...
    function get_estilos($cats="",$order="ASC"){
      $whereclause = "(cats LIKE '%Papel Pintado%' OR cats LIKE '%Foto Murales%') ";
      if($cats=="") return $this->db->from('demo_estilo')->where($whereclause)->where(array('activo'=>1))->order_by('principal','desc')->order_by('estilo_name',$order)->get()->result();
      else return $this->db->from('demo_estilo')->like("cats",$cats,"both")->where(array('activo'=>1))->order_by('principal','desc')->order_by('estilo_name',$order)->get()->result();
    }
