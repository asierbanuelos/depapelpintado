        ...
        */
        if ($row['ord_det_quantity_shipped']>0){
          ...
          */
          ...
          <td width="73" style="border-right: thin solid #CCC;" class="borde-right"><?=round($row[$this->flexi_cart_admin->db_column('order_details', 'item_quantity_cancelled')], 2)?></td>
          ...
          <td colspan="2"><?
          if ($row[$this->flexi_cart_admin->db_column('order_details', 'item_discount_quantity')] > 0){
            $item_bruto_devuelto=$row['ord_det_price'] * $row['ord_det_quantity_cancelled'];
            $item_descontado_devuelto=$row['ord_det_discount_price'] * $row['ord_det_quantity_cancelled'];
            
            echo $negativo.'<span style="text-decoration:line-through;">'.$this->flexi_cart_admin->format_currency($item_bruto_devuelto, TRUE, 2, TRUE).'</span><br/>';
            echo $negativo.$this->flexi_cart_admin->format_currency($item_descontado_devuelto, TRUE, 2, TRUE);
          }
          else{
            $item_bruto_devuelto=$row['db_column'] * $row['ord_det_quantity_cancelled'];
            echo $negativo.$this->flexi_cart_admin->format_currency($item_bruto_devuelto, TRUE, 2, TRUE);
          }
          ?></td>
          </tr>
          <?
...
<?php echo $negativo.$summary_data[$this->flexi_cart_admin->db_column('order_summary', 'reward_voucher_description')];?>
  ...
  /*
  [ord_det_price] => 422.05
  [ord_det_discount_price] => 364.88
  [ord_det_tax] => 73.26
  [ord_det_shipping_rate] => 0.00
  [ord_det_weight] => 4.00
  [ord_det_reward_points] => 4222.00
  */
  $total_pedido_original=$refund_data[0]['ord_det_price'];
  $total_final=$refund_data[0]['ord_det_discount_price'];
  $total_descuento=$total_pedido_original - $total_final;
  $total_envios=$refund_data[0]['ord_det_shipping_rate'];
  $total_envios=0;

  $total_impuestos=$refund_data[0]['ord_det_tax'];
  ...
  <tr>
   ...
   <td class="borde-right">&nbsp;</td>
    <td class="borde-right">
      <?php 
      if ($total_pedido_original > $total_final) 
        echo $negativo.$this->flexi_cart_admin->format_currency($total_descuento, TRUE, 2, TRUE);
      ?>
    </td>
    <td class="borde-right">
      <?php echo $negativo.$this->flexi_cart_admin->format_currency($total_envios, TRUE, 2, TRUE);?>
    </td>
    <td class="borde-right">
      <?php 
