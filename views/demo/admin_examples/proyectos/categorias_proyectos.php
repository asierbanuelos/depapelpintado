...
<head>
	<meta charset="utf-8">
	<title>Categorias proyectos</title>
...
<?php $this->load->view('includes/demo_header'); ?>
  <div class="container pb-4">
    ...
    <div style="font-size:40px;font-weight: 300;color: #B05380;text-align:center;border-bottom: 1px solid #B05380;">LISTADO CATEGORÍAS PROYECTOS</div>
    <div style="height: 20px"></div>
    <div class="sec row">
      <div class="col twelve">
        <span class="edit btn" style='width: 100%;margin:0;'><a style="color: #fff;text-decoration: none;" href='/admin_library/editar_categoria_proyecto'>Nueva categoría</a></span>
      </div>
    </div>
    <div class="list">
      <?php
      ...
      */
      ...
      foreach ($categorias as $key){
        ...
        $a_categorias[$seo_completo][$key->idproyecto_categoria]['proyecto_categoria_name']=$key->proyecto_categoria_name;
        $a_categorias[$seo_completo][$key->idproyecto_categoria]['proyecto_categoria_name_url']=$key->proyecto_categoria_name_url;
        $a_categorias[$seo_completo][$key->idproyecto_categoria]['descripcion_categoria']=$key->descripcion_categoria;
        $a_categorias[$seo_completo][$key->idproyecto_categoria]['estado_texto']=$estado_texto;
        $a_categorias[$seo_completo][$key->idproyecto_categoria]['estado_texto_ok']=$estado_texto_ok;
        $a_categorias[$seo_completo][$key->idproyecto_categoria]['meta_title']=$key->meta_title_categoria;
        $a_categorias[$seo_completo][$key->idproyecto_categoria]['estado_title']=$estado_title;
        $a_categorias[$seo_completo][$key->idproyecto_categoria]['estado_title_ok']=$estado_title_ok;
        $a_categorias[$seo_completo][$key->idproyecto_categoria]['meta_description']=$key->meta_description_categoria;
        $a_categorias[$seo_completo][$key->idproyecto_categoria]['estado_desc']=$estado_desc;
        $a_categorias[$seo_completo][$key->idproyecto_categoria]['estado_desc_ok']=$estado_desc_ok;
        
        $a_categorias[$seo_completo][$key->idproyecto_categoria]['meta_keywords']=$key->meta_keywords_categoria;
        $a_categorias[$seo_completo][$key->idproyecto_categoria]['categoria_publico']=$key->categoria_publico;
      }
      ...
      foreach ($a_categorias as $seo_completo=>$a_datos){
        foreach ($a_datos as $id=>$datos){
          ...
          <div style="border-bottom:dotted 1px #aaa;margin:0;padding:5px;<?php echo $inactivo; ?>" class="sec row" id='registro_<?php echo $id; ?>'>
            <div class="col two">
              <strong><a href='/proyectos/<?php echo $datos['proyecto_categoria_name_url'];?>' target='_blank'><?php echo $datos['proyecto_categoria_name']."&nbsp;".$publico_txt;?></a></strong>
            </div>
            <div class="col three ">
              <?php 
            ...
            <div class="col one">
              <span class="edit btn" id="<?php echo $id; ?>" style='width: 100%;'><a style="color: #fff;text-decoration: none;" href='/admin_library/editar_categoria_proyecto/<?php echo $id; ?>'>Editar</a></span>
            </div>
          </div>
          <?php
        }
