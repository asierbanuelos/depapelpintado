...
<body>
  ...
  <div class="container">
    ...
    <div style="font-size:40px;font-weight: 300;color: #B05380;text-align:center;border-bottom: 1px solid #B05380;">LISTADO INFO SEO COLECCIONES EKAM</div>
    <div style="height: 20px"></div>
    <div class="sec row">
      <div class="col three">&nbsp;</div>
      <div class="col six">
        <span class="edit btn" style='width: 100%;margin:0;'><a style="color: #fff;text-decoration: none;" href='/admin_library/colecciones_seo'>Ver estado de las colecciones principales</a></span>
      </div>
    </div>
    <div class="list">
      <?php
      ...
      foreach ($col as $key){
        if ($key->activo==1 && $key->publico==1 && $key->publico2==1){
          ...
          if (trim($key->col_text_ekam)==''){
            $estado_texto='<span style="color:red;">Sin descripción</span>';
            $seo_completo=0;
          }
          elseif(str_word_count($key->col_text_ekam)<900){
            $estado_texto='<span style="color:red;">Descripción con '.str_word_count($key->col_text_ekam).' palabras</span>';
            $a_descripciones[]=$key->col_text_ekam;
            $seo_completo=0;
          }
          elseif(in_array($key->col_text_ekam, $a_descripciones)){
          ...
          if(in_array($key->meta_titlec_ekam, $a_meta_title)){
            $estado_title.="<span style='color:red;'>Metatitle duplicado</span><br />";
            $seo_completo=0;
          }
          if (trim($key->meta_titlec_ekam)==''){
            $estado_title.='<span style="color:red;">Sin metatitle</span>';
            $seo_completo=0;
          }
          elseif(strlen($key->meta_titlec_ekam)>60){
            $estado_title.='<span style="color:red;">Metatitle > 60 ('.strlen($key->meta_titlec_ekam).')</span>';
            $a_meta_title[]=$key->meta_titlec_ekam;
            $seo_completo=0;
          }
          else{
          ...
          if(in_array($key->meta_descriptionc_ekam, $a_meta_desc)){
            $estado_desc.="<span style='color:red;'>Metadesc duplicado</span><br />";
            $seo_completo=0;
          }
          if (trim($key->meta_descriptionc_ekam)==''){
            $estado_desc.='<span style="color:red;">Sin Metadesc</span>';
            $seo_completo=0;
          }
          elseif(strlen($key->meta_descriptionc_ekam)>150){
            $estado_desc.='<span style="color:red;">Metadesc > 150 ('.strlen($key->meta_descriptionc_ekam).')</span>';
            $a_meta_desc[]=$key->meta_descriptionc_ekam;
            $seo_completo=0;
          }
          else{
          ...
          $a_colecciones[$key->cat_name][$seo_completo][$key->coleccion_id]['col_text_ekam']=$key->col_text_ekam;
          ...
          $a_colecciones[$key->cat_name][$seo_completo][$key->coleccion_id]['meta_title']=$key->meta_titlec_ekam;
          ...
          $a_colecciones[$key->cat_name][$seo_completo][$key->coleccion_id]['meta_description']=$key->meta_descriptionc_ekam;
          ...
          foreach ($a_datos as $id=>$datos){
            ...
            <div style="border-bottom:dotted 1px #aaa" class="sec row" id='registro_<?php echo $id; ?>'>
              ...
              <div class="col three ">
                ...
                echo "<div style='display:none;' id='texto_$id' class='ctex'>{$datos['col_text_ekam']}</div> \n";
                ?>
              </div>
              <div class="col three ">
                <?php 
              ...
              <div class="col one">
                ...
                <span class="edit btn" id="<?php echo $id; ?>" style='width: 100%;'>
                  <a style="color: #fff;text-decoration: none;" href='/admin_library/editar_coleccion_seo/<?php echo $id; ?>/1'>Editar</a>
                </span>
                <?php 
