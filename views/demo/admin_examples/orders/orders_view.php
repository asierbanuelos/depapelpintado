...
<head>
    ...
    <style>
    	.estado a{color:#000;text-decoration: none;width:100%;height: 100%;display: block}
    	.txt-center{text-align:center;}
    	.txt-right{text-align:right;}
    	.pedido-ekam{background-color: #ddd;}
    </style>
			...
			<table>
				<thead>
					<tr>
						...
						<th class="spacer_100 txt-right">Total Artículos</th>
						<th class="spacer_100 txt-right">Total Importe</th>
						<th class="spacer_100 txt-center">Fecha</th>
						<th class="spacer_125 txt-center">Estado</th>
				...
				if (! empty($order_data)){ 	
					foreach ($order_data as $row) { 
						...
						$origen_pedido='';
						$txt_origen_pedido='';
						if ($row['ord_user_ekam_fk']!=0){
							$origen_pedido='pedido-ekam';
							$txt_origen_pedido=' (ekam)';
						}
					?>
					<tr class='<?php echo $origen_pedido; ?>'>
						<td>
							<a href="<?php echo $base_url; ?>admin_library/order_details/<?php echo $order_number; ?>"><?php echo $order_number.$txt_origen_pedido; ?></a>
						</td>
						<td>
						...
						<td class="txt-right">
							<?php 
							//echo $row[$this->flexi_cart_admin->db_column('order_summary', 'total')].' &euro;'; 
							echo number_format($row[$this->flexi_cart_admin->db_column('order_summary', 'total')], '2', ',', '.').' &euro;'; 
							//echo "<br />".$row[$this->flexi_cart_admin->db_column('order_summary', 'ord_user_fk')];
							//echo "<br />".$row[$this->flexi_cart_admin->db_column('order_summary', 'ord_user_ekam_fk')];
							?>
						</td>
						<td class="txt-center">
