...
class Flexi_cart_admin_model extends Flexi_cart_lite_model
{
	...
	private function update_discount_usage($discount_data = FALSE, $order_cancelled = FALSE, $operator = '-')
	{
		...
		if ($this->get_enabled_status('discounts'))
		{
			...
			// Get user email and order_number for usage log.
			$CI =& get_instance();
			$user_email = '';
			...
			$order_number = isset($discount_data['settings']['configuration']['order_number'])
				? $discount_data['settings']['configuration']['order_number'] : '';
			...
			if (! empty($existing_discount_data))
			{
				...
				foreach($existing_discount_data as $discount_id)
				{
					...
					// Remove usage log if order is cancelled/resaved.
					if (! empty($user_email))
					{
						$this->db->where('discount_id', $discount_id)
							->where('user_email', $user_email)
							->delete('discount_usage_log');
					}
				}
			}
