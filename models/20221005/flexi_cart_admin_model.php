...
class Flexi_cart_admin_model extends Flexi_cart_lite_model
{
	...
	###+++++++++++++++++++++++++###
	public function generar_vale($user_id = FALSE, $code = FALSE, $description = FALSE, $expire_days = FALSE){
      ...
      $this->db->trans_start();
		...
		$description = (empty($description)) ? 'Abono por muestra: '.$code : $description;
		...
		$sql_insert = array(
			...
			$tbl_col_discounts['value_discounted'] => 2,
	...
	private function set_order_summary($custom_summary_data)
	{
		...
		//Parche
		$ord_sub_total_aux=$sql_update['ord_sub_total'];
		$zatiak=explode('.', $sql_update['ord_sub_total']);
		if (isset($zatiak[1]))
			$zatiak[1]=substr($zatiak[1], 0, 2);
		$ord_sub_total_aux=implode('.', $zatiak);
		
		$sql_update['ord_sub_total']=$ord_sub_total_aux;
		//~ print '<pre><xmp>';
		//~ print_r($sql_update);
		//~ print '</xmp></pre>';
		//~ exit;
