...
class Demo_cart_admin_model extends CI_Model {
	...
	function get_items_count($fab=0,$colec=0,$todos=0,$precio='', $referencia=''){
		$temp=$this->db->select("*",FALSE)->from('demo_items');
		if($fab!=0)$temp->where('item_cat_fk',$fab);
...
if($data['col']!=0)$itemarray['item_coleccion_id']=$data['col'];
      $lastid=$data['item_id'];
    ...
    function demo_update_gama(){
      $data=$this->input->post(NULL, TRUE);
      $cats=implode(',',$data['cats']);
      $this->db->where('gama_id',$data['gama'])->update('demo_gama',array('gama_name'=>$data['name'],'cats'=>$cats));
    }
