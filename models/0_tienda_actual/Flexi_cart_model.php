...
*/
	...
	function get_item($id=0,$cat=0){
         ...
         if($cat!=5) {
          ...
          ->join('demo_estilo', 'estilo_id = estilo_item_estilo')
			...
			else  $temp = $this->db->select("*",FALSE)->from('demo_items')
                  ->join('(discount_group_items JOIN discounts ON disc_group_item_group_fk = disc_group_fk) ', 'disc_group_item_item_fk = item_id','left')->where('demo_items.activo',1)->where('demo_items.item_id',$id);
            $item=$temp->get()->row_array();
			if(isset($item['item_id'])){
				$deskontu_datuak=$this->get_erregistroak_deskontuak($item['item_id']);
    ...
    function search_items($search="",$page=-1){
     ...
     if(count($exploded)>0){
       foreach( $exploded as $key){
        $s0.=" AND item_ref LIKE '%".$key."%'";
       }
     }
     $temp0->order_by('orden','asc')->order_by('RAND(1)')->group_by('item_id');
      $temp0->where(array('publico'=>1,'publico2'=>1,'publico3'=>1));
      $temp0->where("(".$s0.")");
	  
	  
      if($page>0)$temp0->limit(40,$page*40);
      else $temp0->limit(40,0); 
     ...
     if(count($exploded)>0){
      ...
      $temp->order_by('orden','asc')->order_by('RAND(1)')->group_by('item_id');
      $temp->where(array('publico'=>1,'publico2'=>1,'publico3'=>1));
      $temp->where("(".$s.")");
      if($page>0)$temp->limit(40,$page*40);
      else $temp->limit(40,0); 
      $result=$temp->order_by('portada','desc')->get()->result_array();
    ...
    */
		...
		*/
		...
		if (count($a_item_idak)!=0){
			...
			*/
			...
			$where_txt=implode(' AND ', $a_where);
			$query=$this->db->query("SELECT item_cat_fk FROM demo_items WHERE ".$where_txt);
			$result=$query->result_array();
			...
			foreach ($result as $i=>$ezaugarriak){
				if (isset($kategoria_datuak[$ezaugarriak['item_cat_fk']])){
					...
					if (!isset($a_markak[$marca_txt]['cont']))
						$a_markak[$marca_txt]['cont']=1;
					else
						$a_markak[$marca_txt]['cont']++;
				}
			}
			ksort($a_markak);
			...
			*/
			...
			$a_where=array();
			if (count($a_items_marcas_seleccionadas)==0 && count($a_items_colores_seleccionados)==0)
				$a_where[]=' estilo_item_item IN ('.implode(',', $a_item_idak).')';
			else{
				if (count($a_items_marcas_seleccionadas))
			...
			$where_txt=implode(' AND ', $a_where);
			$query=$this->db->query("SELECT estilo_item_estilo FROM demo_estilo_item WHERE ".$where_txt);
			$result=$query->result_array();
			...
			foreach ($result as $i=>$ezaugarriak){
				if (isset($estilos[$ezaugarriak['estilo_item_estilo']])){
					...
					if (!isset($a_estiloak[$estilo_txt]['cont']))
						$a_estiloak[$estilo_txt]['cont']=1;
					else
						$a_estiloak[$estilo_txt]['cont']++;
				}
			}
			ksort($a_estiloak);
	        ...
	        */
			...
			$a_where=array();
			if (count($a_items_marcas_seleccionadas)==0 && count($a_items_estilos_seleccionados)==0)
				$a_where[]=' gama_item_item IN ('.implode(',', $a_item_idak).')';
			else{
				if (count($a_items_marcas_seleccionadas))
			...
			$where_txt=implode(' AND ', $a_where);
			$query=$this->db->query("SELECT gama_item_gama FROM demo_gama_item WHERE ".$where_txt);
			$result=$query->result_array();
			...
			foreach ($result as $i=>$ezaugarriak){
				if (isset($colores[$ezaugarriak['gama_item_gama']])){
					...
					if (!isset($a_koloreak[$color_txt]['cont']))
						$a_koloreak[$color_txt]['cont']=1;
					else
						$a_koloreak[$color_txt]['cont']++;
				}
			}
			ksort($a_koloreak);
	...
	function get_items_filtros_nuevo_listado($tipo_producto,$a_ids,$page=-1, $order=''){
		...
		$order_by=" ORDER BY portada desc, orden asc, RAND(".date("d").")";
		...
		$query=$this->db->query("SELECT * FROM demo_items $join_txt WHERE $where_txt GROUP BY demo_items.item_id $order_by $limit_txt ");
		$result=$query->result_array();
	...
	function get_filtros_nuevo_listado($tipo_producto, $idcoleccion=0){
		$a_kategoriak=$this->get_erregistroak('demo_categories', 'cat_id', array('publico'=>1));
		...
		$temp->where_in('item_cat_fk',$a_kategoriak);
		$temp->where_in('item_coleccion_id',$a_colecciones);
		$temp->where("item_tipo = ".$tipo_producto);
		$temp->where('demo_items.img !=',"");
		$temp->where('demo_items.variante_de',"0");
		...
		// Si la categoría está vacía, no hacemos nada
		if (count($a_item_idak)!=0){
			$filtrar_items_totales=false;
			$a_items_marcas_seleccionadas=array();
			if (isset($_REQUEST['marca']) && trim($_REQUEST['marca'])!=''){
	...
	function get_filtros_categoria_seo($categoria_seo){
		$temp=$this->db->select("nuevacategoria_item_id",FALSE);
		$temp->from('nueva_categoria_item');
		$temp->where('nueva_categoria_id',$categoria_seo->nueva_categoria_id);
	
		//$result=$temp->get()->result_array();
		$result=$temp->get()->result_array();
		$a_ids=array(); // Todos los items de la categoría
		foreach ($result as $i=>$ezaugarriak){
			$a_ids[$ezaugarriak['nuevacategoria_item_id']]=$ezaugarriak['nuevacategoria_item_id'];
		}
		// Si la categoría está vacía, no hacemos nada
		$a_item_idak=array();
		//$a_item_idak[0]=0;
		if (count($a_ids)!=0){
			...
			// Vamos a filtrar dentro de lo asignado a la categoría, los items activos y publicados, de marcas y colecciones activas y publicadas, que no sean variantes de productos
			//$temp=$this->db->select("*",FALSE);
			$temp=$this->db->select("item_id",FALSE);
			$temp->from('demo_items');
			$temp->where(array('demo_items.activo'=>1,'publico3'=>1));
			...
			$result=$temp->get()->result_array();
			// todos los ids activos y publicados
			//$a_item_idak=array();
			foreach ($result as $i=>$ezaugarriak){
				$a_item_idak[$ezaugarriak['item_id']]=$ezaugarriak['item_id'];
			}
	...
	function get_items_categoria_seo_filtros($categoria_seo,$a_ids,$page=-1, $order=''){
		// recibimos todos los ids de la categoría por un filtrado previo
		...
		*/
		//$temp->where('demo_items.item_coleccion_id',1743);
		if (isset($_REQUEST['eneko'])){
			print '<pre><xmp>';
			print_r($a_ids);
			print '</xmp></pret>';
			exit;
		}
		if (!isset($a_ids) || count($a_ids)==0)
			$a_ids[0]=0;
		$temp->where_in('demo_items.item_id',$a_ids);
		$temp->where('demo_items.item_tipo',$categoria_seo->tipo_producto);
		...
		*/
		...
		$registros_por_pagina=42;	
		if($page>0)
			$temp->limit($registros_por_pagina,$page*$registros_por_pagina);
		else 
			$temp->limit($registros_por_pagina,0);
	...
	function get_items_nuevo_listado_filtros($tipo_producto,$a_ids,$page=-1, $order=''){
		// recibimos todos los ids de la categoría por un filtrado previo
		...
		if (count($a_ids)==0)
			$a_ids[]=0;
		$temp->where_in('demo_items.item_id',$a_ids);
		$temp->where('demo_items.item_tipo',$tipo_producto);
		...
		if (isset($_REQUEST['id_coleccion']) && trim($_REQUEST['id_coleccion'])!=0){
			$temp->where('demo_items.item_coleccion_id',$_REQUEST['id_coleccion']);
		}
	...
	function get_items_categoria_seo($categoria_seo,$fab=null,$estilo=null,$gama=null,$page=-1, $order=''){
		$a_kategoriak=$this->get_erregistroak('demo_categories', 'cat_id', array('publico'=>1));
		$a_colecciones=$this->get_erregistroak('demo_coleccion', 'coleccion_id', array('publico2'=>1));

		$temp=$this->db->select("*",FALSE);
		$temp->from('demo_items');
		$temp->join('nueva_categoria_item', 'item_id = nuevacategoria_item_id');
		...
		$temp->where('demo_items.img !=',"");
		$temp->where('demo_items.variante_de',"0");
		$temp->where('nueva_categoria_item.nueva_categoria_id',$categoria_seo->nueva_categoria_id);
      ...
      if ($fab!=null){
        ...
        $whereclause = "(ccats LIKE '%".$categ."%') ";
         ...
         $temp->where($whereclause);
           ...
           return $temp->where('coleccion_cat_id',$fab)->where(array('activo'=>1,'publico2'=>1))->order_by("coleccion_name","ASC")->get()->result_array();
      }
    }
    private function get_item_data($item = FALSE, $item_data = FALSE, $insert = FALSE, $force_column_update = FALSE)
	{
    ...
    function get_categoriessample($cats=""){
      $whereclause = "(cats LIKE '%Papel Pintado%' OR cats LIKE '%Foto Murales%') ";
      if($cats=="")return $this->db/*->select("cat_name AS n, cat_id AS i,",FALSE)*/->from('demo_categories')->where($whereclause)->where(array('activo'=>1,'publico'=>1))->limit(16)->order_by('cat_name','RAND')->get()->result();
      else return $this->db/*->select("cat_name AS n, cat_id AS i,",FALSE)*/->from('demo_categories')->like('cats',$cats,'both')->where(array('activo'=>1,'publico'=>1))->limit(16)->order_by('cat_name','RAND')->get()->result();
    ...
    function get_colecciones($cat=0,$order="ASC"){
      if ($cat==0 )return $this->db->from('demo_coleccion')->join('demo_categories', 'coleccion_cat_id = cat_id')->where(array('demo_coleccion.activo'=>1,'publico'=>1,'publico2'=>1))->order_by('coleccion_name',$order)->get()->result();
      else return $this->db->from('demo_coleccion')->join('demo_categories', 'coleccion_cat_id = cat_id')->where(array('coleccion_cat_id'=>$cat,'demo_coleccion.activo'=>1,'publico'=>1,'publico2'=>1))->order_by('coleccion_name',$order)->get()->result();
    }
    ...
    function get_filtros_categoria(){
     	return $this->db->from('nueva_categoria_familia')->order_by('orden_cat_familia','nombre_familia_categoria','ASC','ASC')->get()->result();
    }
