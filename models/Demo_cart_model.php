...
class Demo_cart_model extends CI_Model {
	...
	function demo_insert_ajax_link_item_to_cart($item_id = 0)
	{
		...
		if ($item_id == 1)
		{
			...
			$cart_data = array('id' => 1, 'name' => 'Item #501, single item via an ajax link.', 'quantity' => 1, 'price' => 30);
		}
		else if ($item_id == 502)
		{
	...
	function demo_insert_database_item_to_cart($item_id = 0)
	{
      ...
      if (isset($_POST['item_extra']) && trim($_POST['item_extra'])!=''){
      	$extra_info_item=trim($_POST['item_extra'], ',,');
      	$a_extra_zatiak=explode(',,', $extra_info_item);
      	$extra_info_item="<br />(".implode(' - ',$a_extra_zatiak).')';
      }
      $precio_unitario_final=0;
      if (isset($_POST['precio_unitario_final']))
        $precio_unitario_final=  $_POST['precio_unitario_final'];
      $precio_unitario_final_sin_desc=0;
      if (isset($_POST['precio_unitario_final_sin_desc']))
        $precio_unitario_final_sin_desc=  $_POST['precio_unitario_final_sin_desc'];

		$this->load->library('flexi_cart');
		$query = $this->db->select('item_name, item_price, item_weight,item_ref,img,item_cat_fk,item_tipo')
			->from('demo_items')
		...
		if ($query->num_rows() == 1){
			...
			if($this->input->post('ud')=="m2"){
				$nombre_completo=$fab." - ". $col." - ".$item_data['item_ref']. " (".$this->input->post('p_ancho_solicitado')."x".$this->input->post('p_alto_solicitado')."m)".$extra_info_item;
				if (isset($_POST['texto_parentesis_carrito']) && trim($_POST['texto_parentesis_carrito'])!='')
					$nombre_completo=$fab." - ". $col." - ".$item_data['item_ref']." ".$_POST['texto_parentesis_carrito'].$extra_info_item;

              $cart_data = array(
				'id' => $item_id/*+200000+((($this->input->post('p_ancho')*100)+($this->input->post('p_alto')*100))*10000)*/, 
				//'id' => $item_id+200000+((($this->input->post('p_ancho_solicitado')*100)+($this->input->post('p_alto_solicitado')*100))*10000), 
				//~ 'name' => $fab." - ". $col." - ".$item_data['item_ref']. "(".$this->input->post('p_ancho')."x".$this->input->post('p_alto')."m)", 
				//'name' => $fab." - ". $col." - ".$item_data['item_ref']. "(".$this->input->post('p_ancho_solicitado')."x".$this->input->post('p_alto_solicitado')."m)".$extra_info_item, 
				'options' => array(
					'Ancho' => $this->input->post('p_ancho_solicitado'), 'Alto' => $this->input->post('p_alto_solicitado')
				),
				'name' => $nombre_completo, 
				'quantity' => $qty, 
				//'price' => $item_data['item_price']*$this->input->post('p_ancho')*$this->input->post('p_alto'), 
				'price' => $precio_unitario_final_sin_desc, 
				'precio_unitario_final' => $precio_unitario_final,
				'precio_unitario_final_sin_desc' => $precio_unitario_final_sin_desc,
				'weight' => $item_data['item_weight'],
                'thumb' => $item_data['img'],
                'idmarca'=> $item_data['item_cat_fk'],
                'tipo'=> $item_data['item_tipo']
                );
            }
            else if( $this->input->post('sample')==1){
              $cart_data = array(
				'id' => $item_id+100000, 
				'name' => $fab." - ". $col." - ".$item_data['item_ref']." muestra", 
				'quantity' => $qty, 
				'price' => 2, 
				'weight' => 0,
                'thumb' => $item_data['img'],
            ...
            //else if($col==""){
            else if($item_data['item_tipo']==5){
                $cart_data = array(
				'id' => $item_id, 
				'name' => "Herramientas - ". $item_data['item_name']." - ".$item_data['item_ref'], 
				'quantity' => $qty, 
                ...
                $cart_data = array(
				'id' => $item_id, 
				'name' => $fab." - ". $col." - ".$item_data['item_ref'], 
				'quantity' => $qty, 
				'price' => $item_data['item_price'], 
				'precio_unitario_final' => $precio_unitario_final,
				'precio_unitario_final_sin_desc' => $precio_unitario_final_sin_desc,
			...
			$this->flexi_cart->insert_items($cart_data,true);
		}
	...
	function demo_save_order()
	{
		...
		$this->form_validation->set_rules('checkout[billing][name]');
		$this->form_validation->set_rules('checkout[billing][add_01]');
		$this->form_validation->set_rules('checkout[billing][city]');
		$this->form_validation->set_rules('checkout[billing][state]');
		$this->form_validation->set_rules('checkout[billing][post_code]');
		$this->form_validation->set_rules('checkout[billing][country]');
		$this->form_validation->set_rules('checkout[email]', 'Email', 'required|valid_email');
		$this->form_validation->set_rules('checkout[phone]', 'Teléfono', 'required|min_length[9]');
		$this->form_validation->set_rules('checkout[shipping][name]', 'Nombre Destinatario', 'required');
		$this->form_validation->set_rules('checkout[shipping][country]', 'Pais', 'required');
		$this->form_validation->set_rules('checkout[shipping][post_code]', 'Codigo Postal', 'required');
		$this->form_validation->set_rules('checkout[shipping][add_01]', 'Direccion Destinatario', 'required');
		$this->form_validation->set_rules('checkout[shipping][city]', 'Población  ', 'required');
		$this->form_validation->set_rules('checkout[shipping][state]', 'Provincia', 'required');
