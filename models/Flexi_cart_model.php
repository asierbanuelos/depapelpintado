...
*/
	...
	public function calculate_shipping_items($discount_data, $void_ship_discount)
	{
		...
		foreach($this->flexi->cart_contents['items'] as $row_id => $row)
		{
			...
			###+++++++++++++++++++++++++++++++++###
			/*
			if (isset($item_shipping_data))
				echo "<br />item_shipping_data";
			else
				echo "<br /> noitem_shipping_data";
			*/
			...
			// Problema con el isset de una variable vacía
			...
			//	(isset($item_shipping_data) && $item_shipping_data[$this->flexi->cart_database['item_shipping']['columns']['separate']] == 1))
				...
				(isset($item_shipping_data[$this->flexi->cart_database['item_shipping']['columns']['separate']]) && $item_shipping_data[$this->flexi->cart_database['item_shipping']['columns']['separate']] == 1))
			{
	...
	private function get_shipping_item_totals()
	{
		...
		$discounts=0;
	    if(isset($this->flexi->cart_contents['settings']['discounts'])){
	    	$discounts=$this->flexi->cart_contents['settings']['discounts']['data']['item_discount_savings'];
	    }
		$item_summary_total = $this->flexi->cart_contents['summary']['item_summary_total']-$discounts;
	 ...
	 * Also checks if the logged-in user has already used the discount code (1 use per customer).
	 */
	public function validate_discount_codes($discount_codes)
	{
		...
		$valid_discounts = $query->result_array();

		// Discount codes require the user to be logged in.
		$CI =& get_instance();
		$user_email = '';
		if (isset($CI->data['usuario']) && isset($CI->data['usuario']->email) && $CI->data['usuario']->email !== '')
		{
			$user_email = $CI->data['usuario']->email;
		}

		// If user is not logged in (guest), reject discount codes.
		if (empty($user_email))
		{
			$this->set_error_message('discount_login_required', 'config');
			return FALSE;
		}
		...
		{
			$discount_id = $discount[$tbl_cols_discount['id']];

			// Check if this email already used this discount.
			$used_by_email = $this->db->where('discount_id', $discount_id)
				->where('user_email', $user_email)
				->count_all_results('discount_usage_log');

			// Check if this IP already used this discount.
			$used_by_ip = $this->db->where('discount_id', $discount_id)
				->where('user_ip', $user_ip)
				->count_all_results('discount_usage_log');

			if ($used_by_email > 0)
			{
				$this->set_error_message('discount_already_used_by_user', 'config');
			}
			else if ($used_by_ip > 0)
			{
				$this->set_error_message('discount_already_used_by_ip', 'config');
			}
			else
			{
		...
		return (! empty($filtered_discounts)) ? $filtered_discounts : FALSE;
	}
			...
			foreach($item_discount_data as $target_column => $discount_data)
			{
				...
				foreach($discount_data as $discount_id => $discount_cols)
				{
					...
					)
					{
						...
						$repeat_discount = ($discount_cols['recursive'] == 1) ? $applicable_item_quantity / $quantity_required : 1;
	...
	public function update_discount_codes($discount_codes = FALSE)
	{
		...
		//~ if ($discount_codes[0]=='ENEKO'){
			//~ echo "<br />modelo flexi_cart_model";
			//~ exit;
		//~ }
		$unset_status = FALSE;
	...
	/*
    function get_item_siempre($id=0,$cat=0){
    	// Modificación para coger los datos del item independientemente de que esté activo, publicado etc, lo trataremos después en el controlador
		if($cat!=5) {
			$temp = $this->db->select("*,(SELECT GROUP_CONCAT(DISTINCT gama_name SEPARATOR ', ')) AS color,(SELECT GROUP_CONCAT(DISTINCT estilo_name SEPARATOR ', ')) AS estilo",FALSE)
			->from('demo_items')

			->join($this->flexi_cart->db_table('item_stock'), 'item_id = '.$this->flexi_cart->db_column('item_stock', 'item'))
			//->join('(discount_group_items JOIN discounts ON disc_group_item_group_fk = disc_group_fk) ', 'disc_group_item_item_fk = item_id','left')
			...
			->where('demo_gama.activo',1)
			->where('demo_estilo.activo',1)
			//->where('publico',1)
			//->where('publico2',1)
			//->where('publico3',1)
			->where('demo_items.item_id',$id)->group_by('item_id');
		}
		else  
			$temp = $this->db->select("*",FALSE)->from('demo_items')
			->join('(discount_group_items JOIN discounts ON disc_group_item_group_fk = disc_group_fk) ', 'disc_group_item_item_fk = item_id','left')->where('demo_items.activo',1)->where('demo_items.item_id',$id);

		$item=$temp->get()->row_array();
		if(isset($item['item_id'])){
			$a_idak[$item['item_id']]=$item['item_id'];
			//$deskontu_datuak=$this->get_erregistroak_deskontuak($item['item_id']);
			$deskontu_datuak=$this->get_erregistroak_deskontuak($a_idak);
			if (isset($deskontu_datuak[$item['item_id']])){
				$item=array_merge($item, $deskontu_datuak[$item['item_id']]);
			}
		}
		return $item;
	}
	*/
    function get_item_siempre($id=0,$cat=0){
		...
		if($cat!=5) {
			//$temp = $this->db->select("*,(SELECT GROUP_CONCAT(DISTINCT gama_name SEPARATOR ', ')) AS color,(SELECT GROUP_CONCAT(DISTINCT estilo_name SEPARATOR ', ')) AS estilo",FALSE)
			$temp = $this->db->select("*, demo_items.activo as 'activo' ",FALSE)
			->from('demo_items')
			->join($this->flexi_cart->db_table('item_stock'), 'item_id = '.$this->flexi_cart->db_column('item_stock', 'item'))
			->join('demo_modelo', 'item_model_id = modelo_id')
			->where('demo_items.item_id',$id)->group_by('item_id');
		...
		if(isset($item['item_id'])){
			...
			$temp_colores = $this->db->select("GROUP_CONCAT(DISTINCT gama_name SEPARATOR ', ') AS color",FALSE)
				->from('demo_gama_item')
				->join('demo_gama', 'gama_id = gama_item_gama')
				->where('gama_item_item',$item['item_id'])
				->where('demo_gama.activo',1);
			$colores=$temp_colores->get()->row_array();

			$temp_estilos = $this->db->select("GROUP_CONCAT(DISTINCT estilo_name SEPARATOR ', ') AS estilo",FALSE)
				->from('demo_estilo_item')
				->join('demo_estilo', 'estilo_id = estilo_item_estilo')
				->where('estilo_item_item',$item['item_id'])
				->where('demo_estilo.activo',1);
			$estilos=$temp_colores->get()->row_array();

			$item['color']=$colores['color'];
			$item['estilo']=$estilos['estilo'];
		}
		return $item;
	}
    function get_item($id=0,$cat=0){
		...
		if($cat!=5) {
			...
			->join('demo_estilo_item', 'item_id = estilo_item_item')
			->join('demo_estilo', 'estilo_id = estilo_item_estilo')
			->where('demo_items.activo',1)->where('demo_gama.activo',1)->where('demo_estilo.activo',1)
			->where('publico',1)
			->where('publico2',1)
			->where('publico3',1)
			->where('demo_items.item_id',$id)->group_by('item_id');
    ...
    function get_variantes($id){
		//~ return $this->db->select("*",FALSE)->from('demo_items')->where('item_id',$id)->or_where('variante_de',$id)->where('demo_items.activo',1)->get()->result_array();
		return $this->db->select("*",FALSE)->from('demo_items')->where('item_id',$id)->or_where('variante_de',$id)->where('demo_items.activo',1)->where('demo_items.publico3',1)->get()->result_array();
    }
        function get_relacionados($id){
			//~ return $this->db->select("*",FALSE)->from('demo_items')->where('item_id',$id)->or_where('variante_de',$id)->where('demo_items.activo',1)->get()->result_array();
			return $this->db->select("*",FALSE)->from('demo_items')->where('item_id',$id)->or_where('relacionado_con',$id)->where('demo_items.activo',1)->where('demo_items.publico3',1)->get()->result_array();
        }
      	function demo_get_item_data($order="ASC")
	{	
		return $this->db->select("*,(SELECT GROUP_CONCAT(gama_name SEPARATOR ', ') AS color)",FALSE)
			->from('demo_items')
                ->join($this->flexi_cart->db_table('item_stock'), 'item_id = '.$this->flexi_cart->db_column('item_stock', 'item'))
			->join('demo_categories', 'item_cat_fk = cat_id')
            ->join('demo_coleccion', 'item_coleccion_id = coleccion_id')
    ...
    function get_items_cole ($cole="",$todos=false,$categ=-1){
      $temp=$this->db->select("*",FALSE);
      $temp->from('demo_items')
            ->join($this->flexi_cart->db_table('item_stock'), 'item_id = '.$this->flexi_cart->db_column('item_stock', 'item'));
            //        ->join('demo_modelo', 'item_model_id = modelo_id')
			$temp->join('demo_categories', 'item_cat_fk = cat_id');
            $temp->join('demo_coleccion', 'item_coleccion_id = coleccion_id');
            //$temp->join('(discount_group_items JOIN discounts ON disc_group_item_group_fk = disc_group_fk) ', 'disc_group_item_item_fk = item_id','left');
             $temp->where('demo_items.activo',1);
             $temp->where('publico',1);
             $temp->where('publico2',1);
             $temp->where('publico3',1);
             $temp->where('demo_items.img !=',"");
             $temp->where('demo_items.item_price !=',0);
             $temp->where('coleccion_id',"$cole");
            if(!$todos){
            	$temp->where('demo_items.variante_de',"0");
            	$temp->where('demo_items.relacionado_con',"0");
            } 
            if($categ>-1)$temp->where('demo_items.item_tipo',$categ);
            
		if (isset($_REQUEST['estilo']) && trim($_REQUEST['estilo'])!=''){
			$a_estilos=explode(',',$_REQUEST['estilo']);
		...
		if (isset($_REQUEST['color']) && trim($_REQUEST['color'])!=''){
			$a_colores=explode(',',$_REQUEST['color']);
			$temp->join('demo_gama_item', 'item_id = gama_item_item');
			$temp->where_in('gama_item_gama',$a_colores);
		}

            $result=$temp->group_by('item_id')->order_by('item_ref')->get()->result_array();
			$a_item_idak=array();
			foreach ($result as $i=>$ezaugarriak){
				if($categ!=5){
					if (isset($kategoria_datuak[$ezaugarriak['item_cat_fk']])){
						$result[$i]=array_merge($result[$i], $kategoria_datuak[$ezaugarriak['item_cat_fk']]);
					}
					if (isset($colecciones_datuak[$ezaugarriak['item_coleccion_id']])){
						$result[$i]=array_merge($result[$i], $colecciones_datuak[$ezaugarriak['item_coleccion_id']]);
					}
				}
				$a_item_idak[$ezaugarriak['item_id']]=$ezaugarriak['item_id'];
				
			}
			$deskontu_datuak=$this->get_erregistroak_deskontuak($a_item_idak);
			$deskontua_hutsik=array();
			foreach ($deskontu_datuak as $i=>$ezaugarriak){
				foreach ($ezaugarriak as $eremua=>$balioa){
					$deskontua_hutsik[$eremua]='';
				}
				break;
			}
			foreach ($result as $i=>$ezaugarriak){
			if (isset($deskontu_datuak[$ezaugarriak['item_id']])){
				$result[$i]=array_merge($result[$i], $deskontu_datuak[$ezaugarriak['item_id']]);
			}
			else
				$result[$i]=array_merge($result[$i], $deskontua_hutsik);
			}
			return $result;
    }
    function get_items_grecas ($a_colecciones=array(),$categ=-1){
		$temp=$this->db->select("item_id, item_name, item_ref, img",FALSE);
		$temp->from('demo_items');
		$temp->join('demo_categories', 'item_cat_fk = cat_id');
        $temp->join('demo_coleccion', 'item_coleccion_id = coleccion_id');
		$temp->where('demo_items.activo',1);
		...
		$temp->where('demo_items.img !=',"");
		$temp->where('demo_items.item_price !=',0);
		$temp->where_in('coleccion_id',$a_colecciones);
    	$temp->where('demo_items.variante_de',"0");
    	$temp->where('demo_items.relacionado_con',"0");
		if($categ>-1)
			$temp->where('demo_items.item_tipo',$categ);
    ...
    function search_items_autocomplete($search=""){
      ...
      $replace_rep = explode(",","c,ae,oe,a,e,i,o,u,a,e,i,o,u,a,e,i,o,u,y,a,e,i,o,u,a,e,i,o,u,a,e,i,o,u,n,,,");
      $slug = function($str) use ($search_rep, $replace_rep) {
    ...
    function get_estancias_home($tipo_producto = 0, $limit = 4) {
      $sql = "
        SELECT
          ...
          nc.categoria_img,
          (SELECT i.imgamb FROM demo_items i
           JOIN nueva_categoria_item nci ON nci.nuevacategoria_item_id = i.item_id
           WHERE nci.nueva_categoria_id = nc.nueva_categoria_id
             AND i.activo=1 AND i.publico3=1 AND i.imgamb != ''
           ORDER BY i.portada DESC, i.item_id DESC LIMIT 1) AS product_img
        FROM nueva_categoria nc
        WHERE nc.idnueva_categoria_familia = 6
    ...
    function get_items_portada($limit = 8) {
      $sql = "
        SELECT
          ...
          i.item_price_aux, i.item_unidad, i.img, i.imgamb,
          i.portada, i.meta_title,
          c.cat_id, c.cat_name,
          col.coleccion_id, col.coleccion_name,
          GROUP_CONCAT(DISTINCT g.idtonalidad ORDER BY g.idtonalidad SEPARATOR ',') AS tonalidades
        FROM demo_items i
        JOIN demo_categories c ON i.item_cat_fk = c.cat_id
        JOIN demo_coleccion col ON i.item_coleccion_id = col.coleccion_id
        LEFT JOIN demo_gama_item gi ON gi.gama_item_item = i.item_id
        LEFT JOIN demo_gama g ON g.gama_id = gi.gama_item_gama AND g.idtonalidad > 0
        WHERE i.activo = 1 AND i.publico3 = 1 AND c.publico = 1 AND col.publico2 = 1
          AND i.img != '' AND i.item_price > 0
        GROUP BY i.item_id
    ...
    function search_items($search="",$page=-1){
      ...
      $temp0=$this->db->select("*",FALSE);
      $temp0->from('demo_items');
      $temp0->join('item_stock', 'demo_items.item_id = item_stock.stock_item_fk', 'left');
     //->join($this->flexi_cart->db_table('item_stock'), 'item_id = '.$this->flexi_cart->db_column('item_stock', 'item'));
      $temp0->join('demo_categories', 'item_cat_fk = cat_id');
      $temp0->join('demo_coleccion', 'item_coleccion_id = coleccion_id');
      //$temp0->join('(discount_group_items JOIN discounts ON disc_group_item_group_fk = disc_group_fk) ', 'disc_group_item_item_fk = item_id','left');
      $temp0->where(array('demo_items.activo'=>1,'publico'=>1,'publico2'=>1,'publico3'=>1));
      $temp0->where('demo_items.img !=',"");
     $s0="(1=1";
     $s1="(1=1";
     $s2="(1=1";
     $s3="(1=1";
     ...
     if(count($exploded)>0){
       foreach( $exploded as $key){
        ...
        $s0.=" AND item_ref LIKE '%".$escaped_key."%'";
        $s1.=" AND item_name LIKE '%".$escaped_key."%'";
        $s2.=" AND cat_name LIKE '%".$escaped_key."%'";
        $s3.=" AND coleccion_name LIKE '%".$escaped_key."%'";
        $s4.=" AND demo_items.item_id IN (SELECT ei.estilo_item_item FROM demo_estilo_item ei INNER JOIN demo_estilo e ON e.estilo_id=ei.estilo_item_estilo WHERE e.estilo_name LIKE '%".$escaped_key."%' AND e.activo=1)";
        $s5.=" AND demo_items.item_id IN (SELECT nci.nuevacategoria_item_id FROM nueva_categoria_item nci INNER JOIN nueva_categoria nc ON nc.nueva_categoria_id=nci.nueva_categoria_id WHERE nc.nueva_categoria_name LIKE '%".$escaped_key."%' AND nc.nueva_categoria_activo=1)";
       }
     }
     $s0.=")";
     $s1.=")";
     ...
     $s4.=")";
     $s5.=")";
     $s0 = "(".$s0." OR ".$s1." OR ".$s2." OR ".$s3." OR ".$s4." OR ".$s5.")";
     $temp0->order_by('demo_items.orden','asc')->order_by('demo_items.item_id','desc')->group_by('item_id');
      $temp0->where(array('publico'=>1,'publico2'=>1,'publico3'=>1));
      $temp0->where("(".$s0.")");
	  
	  
      if($page>0)$temp0->limit(120,$page*120);
      else $temp0->limit(120,0);
      $result = $temp0->order_by('portada','desc')->get()->result_array();
      ...
      if(count($result)!=0)
     	...
     	// FIN BUSQUEDA POR REFERENCIA
      /*
      $fabs=$this->db->select('cat_id')->from('demo_categories')->like('cat_name',$exploded[0],'both');
      for ($i=1;$i<count($exploded);$i++){
        $fabs->or_like('cat_name',$exploded[$i],'both');
      }
      $fabsget=$fabs->get()->result();
      $colec=$this->db->select('coleccion_id')->from('demo_coleccion')->like('coleccion_name',$exploded[0],'both');
      for ($i=1;$i<count($exploded);$i++){
        $colec->or_like('coleccion_name',$exploded[$i],'both');
      }
      $colecget=$colec->get()->result();
      $estilo=$this->db->select('estilo_id')->from('demo_estilo')->like('estilo_name',$exploded[0],'both');
      for ($i=1;$i<count($exploded);$i++){
        $estilo->or_like('estilo_name',$exploded[$i],'both');
      }
      $estiloget=$estilo->get()->result();
      $gama=$this->db->select('gama_id')->from('demo_gama')->like('gama_name',$exploded[0],'both');
      for ($i=1;$i<count($exploded);$i++){
        $gama->or_like('gama_name',$exploded[$i],'both');
      }
      $gamaget=$gama->get()->result();
      $temp=$this->db->select("*",FALSE);
      $temp->from('demo_items');
      //->join($this->flexi_cart->db_table('item_stock'), 'item_id = '.$this->flexi_cart->db_column('item_stock', 'item'));
      $temp->join('demo_categories', 'item_cat_fk = cat_id');
      $temp->join('demo_coleccion', 'item_coleccion_id = coleccion_id');
      //$temp->join('(discount_group_items JOIN discounts ON disc_group_item_group_fk = disc_group_fk) ', 'disc_group_item_item_fk = item_id','left');
      $temp->where(array('demo_items.activo'=>1,'publico'=>1,'publico2'=>1,'publico3'=>1));
      $temp->where('demo_items.img !=',"");
 	  if(count($a_item_idak)){
		$temp->where_not_in('demo_items.item_id',$a_item_idak);
 	  }
     ...
     */
     /*
      if(count($gamaget)>0){
        $g=array();
        foreach( $gamaget as $key){
          $g[]=$key->gama_id;
        }
        $temp->join('demo_gama_item', 'item_id = gama_item_item');
        $s.= " OR `gama_item_gama` IN (".implode(",", $g).")";
      }
      if(count($estiloget)>0){
        $e=array();
        foreach( $estiloget as $key){
          $e[]=$key->estilo_id;
        }
        $temp->join('demo_estilo_item', 'item_id = estilo_item_item');
        $s.=" OR `estilo_item_estilo` IN (".implode(",", $e).")";
      }
      if(count($colecget)>0){
        $c=array();
        foreach( $colecget as $key){
          $c[]=$key->coleccion_id;
        }
        $s.=" OR  `item_coleccion_id` IN (".implode(",", $c).")";
      }
      if(count($fabsget)>0){
        $f=array();
        foreach( $fabsget as $key){
          $f[]=$key->cat_id;
        }
        $s.=" OR  `item_cat_fk` IN (".implode(",", $f).")";
      }
      $temp->order_by('demo_items.orden','asc')->order_by('RAND(1)')->group_by('item_id');
      $temp->where(array('publico'=>1,'publico2'=>1,'publico3'=>1));
      $temp->where("(".trim($s, ' OR').")");
      if($page>0)
      	$temp->limit(40,$page*40);
      else 
      	$temp->limit(40,0); 
      $result_2=$temp->order_by('portada','desc')->get()->result_array();
      
      $a_item_idak=array();
	  foreach ($result_2 as $i=>$ezaugarriak){
			$a_item_idak[$ezaugarriak['item_id']]=$ezaugarriak['item_id'];
	  }
	  if(isset($a_item_idak)){
		  $deskontu_datuak=$this->get_erregistroak_deskontuak($a_item_idak);
		  foreach ($result_2 as $i=>$ezaugarriak){
			if (isset($deskontu_datuak[$ezaugarriak['item_id']])){
				$result_2[$i]=array_merge($result_2[$i], $deskontu_datuak[$ezaugarriak['item_id']]);
			}
		  }
	  }
		/*
    ...
    /*
    function get_items_filter_originala($fab=null,$estilo=null,$gama=null,$page=-1, $categ=0, $econ=0,$losmas=0,  $order=''){
       //$this->db->cache_on();	
       $temp=$this->db->select("*",FALSE);
            $temp->from('demo_items');
            //->join($this->flexi_cart->db_table('item_stock'), 'item_id = '.$this->flexi_cart->db_column('item_stock', 'item'));
           if($categ!=5){
            ...
            if($categ!=5){$temp->where(array('demo_items.activo'=>1,'publico'=>1,'publico2'=>1,'publico3'=>1));}
            else {$temp->where(array('demo_items.activo'=>1,'publico3'=>1));}
            
            if($categ=="" ||$categ==0) $temp->where("item_tipo < 1");
            else $temp->where('item_tipo',$categ);
            if ($econ==1) $temp->where("item_economico",1);
            else if ($losmas==1) $temp->where("item_top",1);
            ...
            $temp->where('demo_items.img !=',"");
            $temp->where('demo_items.variante_de',"0");
    //  $temp->where('item_cat_fk',1);
      if($fab!=null && $fab!=0){
        $temp->where_in('cat_id',$fab);
      }
      if($estilo!=null&& $estilo!=0)$temp->where_in('estilo_item_estilo',$estilo);
     
      if($gama!=null&& $gama!=0)$temp->where_in('gama_item_gama',$gama);
      if($page>0)$temp->limit(40,$page*40);
      else $temp->limit(40,0);
     //aÃ±adir filtro por colores
      $result=$temp->group_by('item_id')->get()->result_array();
	  //$this->db->cache_off();	
    ...
    */
	...
	function get_erregistroak_eremu_guztiak($taula, $ideremua, $a_where){
		$result= $this->db->from($taula)->where($a_where)->get()->result_array();
        /*
        if (isset($_GET['test']) && $taula=='demo_coleccion'){
			echo "<br />".$this->db->last_query();
			echo "<br />cnt ".count($result);
			print '<pre><xmp>';
			print_r($result);
			print '</xmp></pre>';
	...
	function get_erregistroak_eremu_guztiak_where_in_orokorra($taula, $ideremua, $a_where, $id_where_in='', $a_where_in=array()){
		$aux=$this->db->from($taula);
		$aux->where($a_where);
		if (count($a_where_in))
			$aux->where_in($id_where_in,$a_where_in);
	...
	function get_erregistroak_eremu_guztiak_where_in($taula, $ideremua, $a_where, $a_colecciones=array()){
		$aux=$this->db->from($taula);
		$aux->where($a_where);
		$aux->where_in('item_coleccion_id',$a_colecciones);
	...
	function get_erregistroak_deskontuak($a_item_idak){
		if(count($a_item_idak)>0){
			$aux=$this->db->from('discount_group_items');
			$aux->join('discounts', 'disc_group_item_group_fk = disc_group_fk','left');
			$aux->join('discount_groups', 'disc_group_id = disc_group_item_group_fk','left');
			$aux->where('disc_status',1);
	...
	function get_erregistroak_deskontu_guztiak(){
			$aux=$this->db->from('discount_group_items');
			$aux->join('discounts', 'disc_group_item_group_fk = disc_group_fk','left');
			$aux->join('discount_groups', 'disc_group_id = disc_group_item_group_fk','left');
			$aux->where('disc_status',1);
	...
	function get_items_filter($fab=null,$estilo=null,$gama=null,$page=-1, $categ=0, $econ=0,$losmas=0,  $order=''){
		...
		if($categ!=5){
			$temp->where(array('demo_items.activo'=>1,'publico3'=>1));
			$temp->where_in('item_cat_fk',$a_kategoriak);
			$temp->where_in('item_coleccion_id',$a_colecciones);
			//~ $temp->where(array('demo_items.activo'=>1,'publico'=>1,'publico2'=>1,'publico3'=>1));
		}
		else{
			// Solo se muestran las herramientas que estén marcadas como portada
			$temp->where(array('demo_items.portada'=>1,'demo_items.activo'=>1,'publico3'=>1));
			//~ $temp->where(array('demo_items.activo'=>1,'publico3'=>1));
		}

		if($categ=="" ||$categ==0) 
			//~ $temp->where("item_tipo < 1");
			$temp->where("item_tipo = 0");
		else 
			$temp->where('item_tipo',$categ);
		if ($econ==1) 
			$temp->where("item_economico",1);
		else if ($losmas==1) 
			$temp->where("item_top",1);
		...
		if($estilo!=null&& $estilo!=0)
			$temp->where_in('estilo_item_estilo',$estilo);

		if($gama!=null&& $gama!=0)
			$temp->where_in('gama_item_gama',$gama);
		if($page>0)
			$temp->limit(40,$page*40);
		else 
			$temp->limit(40,0);
	...
	function get_filtros_izquierda_listado($tipo_producto, $nueva_categoria_id=0, $idcoleccion=0, $productos_outlet=false){
		// Caché de 90 segundos para evitar ejecutar 6+ queries en cada petición
		$cache_key = 'filtros_' . md5(serialize(array(
			$tipo_producto, $nueva_categoria_id, $idcoleccion, $productos_outlet,
			isset($_REQUEST['calidad'])  ? $_REQUEST['calidad']  : '',
			isset($_REQUEST['limpieza']) ? $_REQUEST['limpieza'] : '',
		...
		if ($tipo_producto>=0){
			if ($tipo_producto==0){
				...
				$a_where[]=" demo_items.item_tipo=$tipo_producto ";
		}
		

		if ($tipo_producto!=5){
			$a_where[]=' item_cat_fk IN ('.implode(',', $a_kategoriak).') ';
			$a_where[]=' item_coleccion_id IN ('.implode(',', $a_colecciones).') ';
		}
		$a_where[]=" demo_items.img!='' ";
		//$a_where[]=" (demo_items.img!='' OR demo_items.item_cat_fk=276) ";
		$a_where[]=" demo_items.variante_de='0' ";
		$a_where[]=" demo_items.relacionado_con='0' ";
		...
		unset($result);
		/*
		echo "<br />zenbat: ".count($a_item_idak);
		print '<pre><xmp>';
		print_r($a_item_idak);
		print '</xmp></pret>';
		exit;
		*/
		...
		if (count($a_item_idak)!=0){
			$filtrar_items_totales=false;
			$a_items_calidad_seleccionada=array();
			if (isset($_REQUEST['calidad']) && trim($_REQUEST['calidad'])!=''){
				$filtrar_items_totales=true;
				$a_calidades=explode(',',$_REQUEST['calidad']);
				$a_vinilo_be=array();
				foreach ($a_calidades as $calidad_txt){
					if (trim($calidad_txt)=='vinilo')
						$a_vinilo_be[]=1;
					if (trim($calidad_txt)=='tejido-no-tejido')
						$a_vinilo_be[]=0;
				}
			...
			if (isset($_REQUEST['limpieza']) && trim($_REQUEST['limpieza'])!=''){
				$filtrar_items_totales=true;
				$a_limpiezas=explode(',',$_REQUEST['limpieza']);
				$a_limpieza_val=array();
				foreach ($a_limpiezas as $limpieza_txt){
					if (trim($limpieza_txt)=='no-lavable')
						$a_limpieza_val[]=0;
					if (trim($limpieza_txt)=='lavable')
						$a_limpieza_val[]=1;
					if (trim($limpieza_txt)=='esponjable')
						$a_limpieza_val[]=2;
					if (trim($limpieza_txt)=='superlavable')
						$a_limpieza_val[]=3;
				}
			...
			$a_items_marcas_seleccionadas=array();
			if (isset($_REQUEST['marca']) && trim($_REQUEST['marca'])!=''){
				$filtrar_items_totales=true;
				/*
				//$a_marcas=explode(',',$_REQUEST['marca']);
				
				$temp=$this->db->select("item_id",FALSE);
				$temp->from('demo_items');
				//$temp->where_in('item_id',$a_ids);
				$temp->where_in('item_id',$a_item_idak);
			...
			if (isset($_REQUEST['estilo']) && trim($_REQUEST['estilo'])!=''){
				...
				$temp=$this->db->select("estilo_item_item",FALSE);
				$temp->from('demo_estilo_item');
				//$temp->where_in('item_id',$a_ids);
				$temp->where_in('estilo_item_item',$a_item_idak);
				$temp->where_in('estilo_item_estilo',$a_estilos);
				$result=$temp->get()->result_array();
				*/
				$a_where=array();
			...
			if (isset($_REQUEST['color']) && trim($_REQUEST['color'])!=''){
				...
				$temp=$this->db->select("gama_item_item",FALSE);
				$temp->from('demo_gama_item');
				//$temp->where_in('item_id',$a_ids);
				$temp->where_in('gama_item_item',$a_item_idak);
				$temp->where_in('gama_item_gama',$a_colores);
				$result=$temp->get()->result_array();
				*/
				$a_where=array();
			...
			//$colecciones_datuak=$this->get_erregistroak_eremu_guztiak('demo_coleccion', 'coleccion_id', array('publico2'=>1));
			/*
			// Marcas que cumplan los filtros de estilos y colores
			$temp=$this->db->select("item_cat_fk",FALSE);
			$temp->from('demo_items');
			if (count($a_items_estilos_seleccionados)==0 && count($a_items_colores_seleccionados)==0)
				$temp->where_in('item_id',$a_item_idak);
			else{
				if (count($a_items_estilos_seleccionados))
					$temp->where_in('item_id',$a_items_estilos_seleccionados);
				if (count($a_items_colores_seleccionados))
					$temp->where_in('item_id',$a_items_colores_seleccionados);
			}
			$result=$temp->get()->result_array();
			*/
			//if (isset($_GET['eneko'])){
			if ($tipo_producto>=0 && $tipo_producto<=2){
				$a_where=array();
				/*
				if (count($a_items_estilos_seleccionados)==0 && count($a_items_colores_seleccionados)==0 && count($a_items_marcas_seleccionadas)==0)
					$a_where[]=' item_id IN ('.implode(',', $a_item_idak).')';
				else{
					if (count($a_items_estilos_seleccionados))
					...
					if (count($a_items_marcas_seleccionadas))
						$a_where[]=' item_id IN ('.implode(',', $a_items_marcas_seleccionadas).')';
				}
				*/
				$a_item_idak_aux=$a_item_idak;
				if (count($a_items_estilos_seleccionados))
					$a_item_idak_aux=array_intersect($a_item_idak_aux, $a_items_estilos_seleccionados);
				if (count($a_items_colores_seleccionados))
					$a_item_idak_aux=array_intersect($a_item_idak_aux, $a_items_colores_seleccionados);
				if (count($a_items_marcas_seleccionadas))
					$a_item_idak_aux=array_intersect($a_item_idak_aux, $a_items_marcas_seleccionadas);
				...
				$a_kalitateak=array();
				foreach ($result as $i=>$ezaugarriak){
					if ($ezaugarriak['item_vinilo']==1){
						$a_kalitateak['Vinílico']['id']='vinilo';
						$a_kalitateak['Vinílico']['cont']=$ezaugarriak['num'];
					}
					else{
						$a_kalitateak['Tejido no tejido']['id']='tejido-no-tejido';
						$a_kalitateak['Tejido no tejido']['cont']=$ezaugarriak['num'];
					}
				}
				ksort($a_kalitateak);
			...
			if ($nueva_categoria_id!=0 && $tipo_producto>=0 && $tipo_producto<=1){
				...
				*/
				$a_limpieza=array();
				foreach ($result as $i=>$ezaugarriak){
					switch ($ezaugarriak['item_lavable']) {
						case '0':
							$a_limpieza['No Lavable']['id']='no-lavable';
							$a_limpieza['No Lavable']['cont']=$ezaugarriak['num'];
							break;
						case '1':
							$a_limpieza['Lavable']['id']='lavable';
							$a_limpieza['Lavable']['cont']=$ezaugarriak['num'];
							break;
						case '2':
							$a_limpieza['Esponjable']['id']='esponjable';
							$a_limpieza['Esponjable']['cont']=$ezaugarriak['num'];
							break;
						case '3':
							$a_limpieza['SuperLavable']['id']='superlavable';
							$a_limpieza['SuperLavable']['cont']=$ezaugarriak['num'];
							break;
					}
				}
				ksort($a_limpieza);
			...
			*/
			...
			foreach ($result as $ezaugarriak){
				if (isset($kategoria_datuak[$ezaugarriak['item_cat_fk']])){
					$marca_txt=$kategoria_datuak[$ezaugarriak['item_cat_fk']];
					$a_markak[$marca_txt]['id']=$ezaugarriak['item_cat_fk'];
					$a_markak[$marca_txt]['cont']=(int)$ezaugarriak['cont'];
				}
			}
			unset($result);
			ksort($a_markak);
			$filtros['marcas']=$a_markak;

			// Estilos que cumplan los filtros de marcas y colores
			/*
			$temp=$this->db->select("estilo_item_estilo",FALSE);
			$temp->from('demo_estilo_item');
			$temp->where_in('estilo_item_item',$a_item_idak);
			if (count($a_items_marcas_seleccionadas))
				$temp->where_in('estilo_item_item',$a_items_marcas_seleccionadas);
			if (count($a_items_colores_seleccionados))
				$temp->where_in('estilo_item_item',$a_items_colores_seleccionados);
			$result=$temp->get()->result_array();
			*/
			...
			*/
			...
			$a_estiloak=array();
			foreach ($result as $ezaugarriak){
				if (isset($estilos[$ezaugarriak['estilo_item_estilo']])){
					$estilo_txt=$estilos[$ezaugarriak['estilo_item_estilo']]->estilo_name;
					$a_estiloak[$estilo_txt]['id']=$ezaugarriak['estilo_item_estilo'];
					$a_estiloak[$estilo_txt]['cont']=(int)$ezaugarriak['cont'];
				}
			}
			unset($result);
			ksort($a_estiloak);
			$filtros['estilos']=$a_estiloak;
			
			
			// Colores que cumplan los filtros de marcas y estilos
			/*
			$temp=$this->db->select("gama_item_gama",FALSE);
			$temp->from('demo_gama_item');
			$temp->where_in('gama_item_item',$a_item_idak);
			if (count($a_items_marcas_seleccionadas))
				$temp->where_in('gama_item_item',$a_items_marcas_seleccionadas);
			if (count($a_items_estilos_seleccionados))
				$temp->where_in('gama_item_item',$a_items_estilos_seleccionados);
			$result=$temp->get()->result_array();
	        */
			...
			*/
			...
			foreach ($result as $ezaugarriak){
				if (isset($colores[$ezaugarriak['gama_item_gama']])){
					$color_txt=$colores[$ezaugarriak['gama_item_gama']];
					$a_koloreak[$color_txt]['id']=$ezaugarriak['gama_item_gama'];
					$aux_colores_seleccion[$ezaugarriak['gama_item_gama']]=$ezaugarriak['gama_item_gama'];
					$a_koloreak[$color_txt]['cont']=(int)$ezaugarriak['cont'];
				}
			}
			unset($result);
			ksort($a_koloreak);
			$filtros['colores']=$a_koloreak;
			...
			$a_tonalidades=array();
			foreach ($result as $i=>$ezaugarriak){
				$a_tonalidades[$ezaugarriak['idtonalidad']][]=$ezaugarriak['gama_id'];
			}
			ksort($a_koloreak);
			...
			if (count($a_items_marcas_seleccionadas))
				$a_item_idak=array_intersect($a_item_idak, $a_items_marcas_seleccionadas);
			if (count($a_items_estilos_seleccionados))
				$a_item_idak=array_intersect($a_item_idak, $a_items_estilos_seleccionados);
			if (count($a_items_colores_seleccionados))
				$a_item_idak=array_intersect($a_item_idak, $a_items_colores_seleccionados);
			if (count($a_items_calidad_seleccionada))
				$a_item_idak=array_intersect($a_item_idak, $a_items_calidad_seleccionada);
			if (count($a_items_limpieza_seleccionada))
				$a_item_idak=array_intersect($a_item_idak, $a_items_limpieza_seleccionada);


			$filtros['ids_categoria']=$a_item_idak; // todos los ids que cumplen los filtros
			$filtros['menu_categorias_seo']=$this->get_categorias_seo_menu_filtrado_por_items($tipo_producto, $a_item_idak);
		}
		else{
			$a_item_idak[0]=0; // todos los ids que cumplen los filtros
			$filtros['ids_categoria']=$a_item_idak; // todos los ids que cumplen los filtros
			$filtros['menu_categorias_seo']=array();
		}
	...
	function get_items_filtros_nuevo_listado($tipo_producto,$a_ids,$page=-1, $order='', $cat_seo_ambiente=0){
		...
		$cache_key = 'items_' . md5(serialize(array(
			$tipo_producto, count($a_ids), $page, $order, $cat_seo_ambiente,
			isset($_REQUEST['calidad'])      ? $_REQUEST['calidad']      : '',
			isset($_REQUEST['limpieza'])     ? $_REQUEST['limpieza']     : '',
			isset($_REQUEST['marca'])        ? $_REQUEST['marca']        : '',
			isset($_REQUEST['estilo'])       ? $_REQUEST['estilo']       : '',
		...
		if ($tipo_producto!=5){
			if (isset($_REQUEST['calidad']) && trim($_REQUEST['calidad'])!=''){
				...
				$a_where[]=' demo_items.item_vinilo IN ('.implode(',',$a_vinilo_be).') ';
				//$a_where[]=' demo_items.item_vinilo IN ('.$_REQUEST['marca'].') ';
			}
			if (isset($_REQUEST['marca']) && trim($_REQUEST['marca'])!=''){
				$a_where[]=' demo_items.item_cat_fk IN ('.$_REQUEST['marca'].') ';
			}
			if (isset($_REQUEST['estilo']) && trim($_REQUEST['estilo'])!=''){
				$a_where[]=' estilo_item_estilo IN ('.$_REQUEST['estilo'].') ';
				$a_join[]=' LEFT JOIN demo_estilo_item ON demo_items.item_id=estilo_item_item ';
			}
			if (isset($_REQUEST['color']) && trim($_REQUEST['color'])!=''){
				$a_where[]=' gama_item_gama IN ('.$_REQUEST['color'].') ';
				$a_join[]=' LEFT JOIN demo_gama_item ON demo_items.item_id=gama_item_item ';
			}
			if ($cat_seo_ambiente!=0){
				$a_join[]=' LEFT JOIN demo_item_cat_ambiente ON demo_items.item_id=demo_item_cat_ambiente.iditem AND idcategoria='.$cat_seo_ambiente.' ';
			}
			if (isset($_REQUEST['id_coleccion']) && trim($_REQUEST['id_coleccion'])!=0){
				$a_where[]=" demo_items.item_coleccion_id={$_REQUEST['id_coleccion']} ";
			}
		}
		...
		if($page>0)
			...
			$limit_txt=" LIMIT $registros_por_pagina OFFSET 0";

		// Mostrar un solo modelo (diseño) por grupo de colores, escogiendo
		...
		foreach ($result as $i=>$ezaugarriak){
			$a_ids_categorias_presentes[$ezaugarriak['item_cat_fk']]=$ezaugarriak['item_cat_fk'];
			$a_ids_colecciones_presentes[$ezaugarriak['item_coleccion_id']]=$ezaugarriak['item_coleccion_id'];
		}
        

        if (isset($_GET['test'])){
			echo "<br />cat_seo_ambiente. ".$cat_seo_ambiente;
			echo "<br />1. ".$query_txt;
		...
		//$kategoria_datuak=$this->get_erregistroak_eremu_guztiak('demo_categories', 'cat_id', array('publico'=>1, 'activo'=>1));
		//$colecciones_datuak=$this->get_erregistroak_eremu_guztiak('demo_coleccion', 'coleccion_id', array('publico2'=>1, 'activo'=>1));
		$kategoria_datuak=$this->get_erregistroak_eremu_guztiak_where_in_orokorra('demo_categories', 'cat_id', array('publico'=>1, 'activo'=>1), 'cat_id', $a_ids_categorias_presentes);
        $colecciones_datuak=$this->get_erregistroak_eremu_guztiak_where_in_orokorra('demo_coleccion', 'coleccion_id', array('publico2'=>1, 'activo'=>1), 'coleccion_id', $a_ids_colecciones_presentes);
		...
		if (!empty($a_item_idak)) {
			...
			")->result_array();
			$a_ton = array();
			foreach ($ton_rows as $row)
				$a_ton[$row['item_id']] = $row['tonalidades'];
			foreach ($result as $i => $item)
				$result[$i]['tonalidades'] = isset($a_ton[$item['item_id']]) ? $a_ton[$item['item_id']] : '';
		}
	...
	function count_items_filtros_nuevo_listado($tipo_producto, $a_ids, $cat_seo_ambiente=0){
		...
		if ($tipo_producto!=5){
			if (isset($_REQUEST['calidad']) && trim($_REQUEST['calidad'])!=''){
				...
				$a_vinilo_be=array();
				foreach ($a_calidades as $calidad_txt){
					if (trim($calidad_txt)=='vinilo') $a_vinilo_be[]=1;
					if (trim($calidad_txt)=='tejido-no-tejido') $a_vinilo_be[]=0;
				}
				$a_where[]=' demo_items.item_vinilo IN ('.implode(',',$a_vinilo_be).') ';
			}
			if (isset($_REQUEST['marca']) && trim($_REQUEST['marca'])!='')
    ...
    function get_fab($fab=null){
      if ($fab!=null){
        $temp=$this->db->select("*",FALSE);
        $temp->from('demo_categories');
           return $temp->where(array('cat_id'=>$fab,'publico'=>1))->get()->row();
      }
    }
    function get_col($fab=null,$categ="0"){
      if ($fab!=null){
        $temp=$this->db->select("*",FALSE);
        $temp->from('demo_coleccion');
        if ($categ==0)$categ="0";if ($categ==1)$categ="1";if ($categ==2)$categ="2";
        if ($categ==3)$categ="3";if ($categ==4)$categ="4";if ($categ==5)$categ="5";
        if ($categ==-1){
          $categ="-1";
          $whereclause = "(ccats IS NOT NULL) ";
        }else{
        $whereclause = "(ccats LIKE '%".$categ."%') ";
        }
         $temp->where($whereclause);
         
           return $temp->where('coleccion_cat_id',$fab)->where(array('activo'=>1,'publico2'=>1))->order_by("orden","ASC")->order_by("coleccion_name","ASC")->get()->result_array();
      }
    }
    private function get_item_data($item = FALSE, $item_data = FALSE, $insert = FALSE, $force_column_update = FALSE)
	{
    ...
    function get_categoriessample($cats=""){
      $whereclause = "(cats LIKE '%Papel Pintado%' OR cats LIKE '%Foto Murales%') ";
      if($cats=="")return $this->db/*->select("cat_name AS n, cat_id AS i,",FALSE)*/->from('demo_categories')->where($whereclause)->where(array('activo'=>1,'publico'=>1))->limit(16)->order_by('cat_name','RANDOM')->get()->result();
      else return $this->db/*->select("cat_name AS n, cat_id AS i,",FALSE)*/->from('demo_categories')->like('cats',$cats,'both')->where(array('activo'=>1,'publico'=>1))->limit(16)->order_by('cat_name','RANDOM')->get()->result();
      
    }
    function get_category($catid){
      return $this->db->from('demo_categories')->where(array('cat_id'=>$catid,'publico'=>1))->get()->result();
    }
    ...
    function get_all_cat_array($order="ASC"){
      $res= array();
      //$result=$this->get_gamas($order);
      $result=$this->db->from('demo_categories')->where(array('activo'=>1))->order_by('cat_name',$order)->get()->result();
    ...
    function get_colecciones($cat=0,$order="ASC"){
      if ($cat==0 )return $this->db->from('demo_coleccion')->join('demo_categories', 'coleccion_cat_id = cat_id')->where(array('demo_coleccion.activo'=>1,'publico'=>1,'publico2'=>1))->order_by('demo_coleccion.orden','ASC')->order_by('coleccion_name',$order)->get()->result();
      else return $this->db->from('demo_coleccion')->join('demo_categories', 'coleccion_cat_id = cat_id')->where(array('coleccion_cat_id'=>$cat,'demo_coleccion.activo'=>1,'publico'=>1,'publico2'=>1))->order_by('demo_coleccion.orden','ASC')->order_by('coleccion_name',$order)->get()->result();
    }
    function get_coleccion($coleccion=0){
      if ($coleccion!=0 )return $this->db->from('demo_coleccion')->join('demo_categories', 'coleccion_cat_id = cat_id')->where(array('coleccion_id' => $coleccion,'publico'=>1,'publico2'=>1))->get()->result();
    ...
    function get_gamas($cats="",$order="ASC"){
    	//$this->db->cache_on();
      if($cats=="")$cats="Papel Pintado";
      $result=$this->db->from('demo_gama')->like("cats",$cats,"both")->where(array('activo'=>1))/*->order_by('gama_principal','desc')*/->order_by('gama_name',$order)->get()->result();
	 //$this->db->cache_off();
	 return $result;    
	}
    ...
    function get_gama_array($order="ASC"){
      $res= array();
      //$result=$this->get_gamas($order);
      $result=$this->db->from('demo_gama')->where(array('activo'=>1))->order_by('gama_name',$order)->get()->result();
    ...
    function get_estilos($cats="",$order="ASC"){
    	//$this->db->cache_on();
      $whereclause = "(cats LIKE '%Papel Pintado%') ";
      if($cats=="") $result=$this->db->from('demo_estilo')->where($whereclause)->where(array('activo'=>1))->order_by('principal','desc')->order_by('estilo_name',$order)->get()->result();
      else $result=$this->db->from('demo_estilo')->like("cats",$cats,"both")->where(array('activo'=>1))->order_by('estilo_name',$order)->get()->result();
    	//$this->db->cache_off();
		return $result;
	}
    function get_estilo_guztiak(){
		$res= array();
		$result=$this->db->from('demo_estilo')->where(array('activo'=>1))->order_by('principal','desc')->order_by('estilo_name','ASC')->get()->result();
		foreach($result as $row){
			$res[$row->estilo_id]=$row;
		}
		return $res;
	}
    ...
    function get_estilo_array($order="ASC"){
      ...
      $result=$this->db->from('demo_estilo')->where(array('activo'=>1))->order_by('principal','desc')->order_by('estilo_name',$order)->get()->result();
      foreach($result as $row){
    ...
    function get_tipo_producto_seo($id){
      return $this->db->from('tipo_producto')->where(array('tipo_producto_id'=>$id))->get()->result();
    }

    function get_categoria_seo($catid){
      return $this->db->from('nueva_categoria')->where(array('nueva_categoria_id'=>$catid))->get()->result();
    }
    function get_categoria_seo_by_name($url){
      return $this->db->from('nueva_categoria')->where(array('nueva_categoria_name_url'=>$url))->get()->result();
    }

    function get_categorias_seo($tipo_producto=-1){
      if ($tipo_producto>=0 )
      	return $this->db->from('nueva_categoria')->where(array('tipo_producto'=>$tipo_producto, 'nueva_categoria_activo'=>1, 'categoria_publico'=>1))->order_by('nueva_categoria_name','ASC')->get()->result();
      else 
      	return $this->db->from('nueva_categoria')->where(array('nueva_categoria_activo'=>1, 'categoria_publico'=>1))->order_by('tipo_producto, nueva_categoria_name','ASC, ASC')->get()->result();
    }

	function get_categorias_seo_menu($tipo_producto=-1, $idnueva_categoria_familia=0){
		$cache_key = 'catmenu_' . md5(serialize(array($tipo_producto, $idnueva_categoria_familia)));
		$cache_file = APPPATH . 'cache/' . $cache_key . '.cache';
		if (file_exists($cache_file) && (time() - filemtime($cache_file)) < 300) {
			$cached = @unserialize(@file_get_contents($cache_file));
			if ($cached !== false) return $cached;
		...
		$f_comp=$this->get_filtros_categoria_array_completo();
		$result=$this->get_categorias_seo($tipo_producto);
		$res = [];
		$res_ord = [];
		foreach($result as $row){
			if ($idnueva_categoria_familia==0){
				if($row->nueva_categoria_activo && $row->idnueva_categoria_familia!=0){
					/*
					$res[$a_tipos[$row->tipo_producto].' '.$familias[$row->idnueva_categoria_familia]][$row->nueva_categoria_id]['nombre']=$row->nueva_categoria_name;
					$res[$a_tipos[$row->tipo_producto].' '.$familias[$row->idnueva_categoria_familia]][$row->nueva_categoria_id]['url']=$row->nueva_categoria_name_url;
					$res[$a_tipos[$row->tipo_producto].' '.$familias[$row->idnueva_categoria_familia]][$row->nueva_categoria_id]['img']=$row->categoria_img;
					*/
					
					$idfamilia=$row->idnueva_categoria_familia;
					$res_ord[$f_comp[$idfamilia]['orden'].'__'.$a_tipos[$row->tipo_producto].' '.$f_comp[$idfamilia]['nombre']][$row->nueva_categoria_id]['nombre']=$row->nueva_categoria_name;
					$res_ord[$f_comp[$idfamilia]['orden'].'__'.$a_tipos[$row->tipo_producto].' '.$f_comp[$idfamilia]['nombre']][$row->nueva_categoria_id]['nombre_filtro']=$row->nombre_filtro;
					$res_ord[$f_comp[$idfamilia]['orden'].'__'.$a_tipos[$row->tipo_producto].' '.$f_comp[$idfamilia]['nombre']][$row->nueva_categoria_id]['url']=$row->nueva_categoria_name_url;
					$res_ord[$f_comp[$idfamilia]['orden'].'__'.$a_tipos[$row->tipo_producto].' '.$f_comp[$idfamilia]['nombre']][$row->nueva_categoria_id]['img']=$row->categoria_img;
				}
			}
			else{
				if($row->nueva_categoria_activo && $row->idnueva_categoria_familia==$idnueva_categoria_familia){
					$res[$a_tipos[$row->tipo_producto].' '.$familias[$row->idnueva_categoria_familia]][$row->nueva_categoria_id]['nombre']=$row->nueva_categoria_name;
					$res[$a_tipos[$row->tipo_producto].' '.$familias[$row->idnueva_categoria_familia]][$row->nueva_categoria_id]['nombre_filtro']=$row->nombre_filtro;
					$res[$a_tipos[$row->tipo_producto].' '.$familias[$row->idnueva_categoria_familia]][$row->nueva_categoria_id]['url']=$row->nueva_categoria_name_url;
					$res[$a_tipos[$row->tipo_producto].' '.$familias[$row->idnueva_categoria_familia]][$row->nueva_categoria_id]['img']=$row->categoria_img;
				}
			}
		}
		//if (isset($_GET['eneko']) && isset($res_ord)){
		if (isset($res_ord)){
			ksort($res_ord);
			//unset($res);
			foreach($res_ord as $familia_ordenada=>$a_datos){
				$a_zatiak=explode('__', $familia_ordenada);
				$res[$a_zatiak[1]]=$a_datos;
			}
			/*
			print '<pre><xmp>';
			print_r($res);
		...
		exit;
		*/
		@file_put_contents($cache_file . '.tmp', serialize($res));
		@rename($cache_file . '.tmp', $cache_file);
		return $res;
    }

    function get_categorias_seo_menu_filtrado_por_items($tipo_producto=-1, $a_item_ids=array()){
		$res = array();
		if (!is_array($a_item_ids) || !count($a_item_ids))
			return $res;

		$a_item_ids = array_values(array_unique(array_map('intval', $a_item_ids)));
		$a_tipos[0]='Papel Pintado';
		$a_tipos[1]='Murales';
		$a_tipos[2]='Revestimientos';
		$a_tipos[3]='Telas';
		...
		$ids_str = implode(',', $a_item_ids);
		$tipo_where = ($tipo_producto >= 0) ? ' AND nc.tipo_producto = '.(int)$tipo_producto : '';
		$sql = "SELECT nc.nueva_categoria_id, nc.nueva_categoria_name, nc.nueva_categoria_name_url,
					nc.categoria_img, nc.nombre_filtro, nc.idnueva_categoria_familia, nc.tipo_producto
				FROM nueva_categoria nc
				INNER JOIN nueva_categoria_item nci ON nci.nueva_categoria_id = nc.nueva_categoria_id
				WHERE nc.nueva_categoria_activo = 1
				AND nc.categoria_publico = 1
				AND nc.idnueva_categoria_familia != 0
				AND nci.nuevacategoria_item_id IN ($ids_str)
				$tipo_where
				GROUP BY nc.nueva_categoria_id
				ORDER BY nc.nueva_categoria_name ASC";
		$result = $this->db->query($sql)->result();

		foreach($result as $row){
			$idfamilia = (int)$row->idnueva_categoria_familia;
			if (!isset($f_comp[$idfamilia]) || !isset($a_tipos[$row->tipo_producto]))
				continue;

			$key = $f_comp[$idfamilia]['orden'].'__'.$a_tipos[$row->tipo_producto].' '.$f_comp[$idfamilia]['nombre'];
			$res[$key][$row->nueva_categoria_id]['nombre']=$row->nueva_categoria_name;
			$res[$key][$row->nueva_categoria_id]['nombre_filtro']=$row->nombre_filtro;
			$res[$key][$row->nueva_categoria_id]['url']=$row->nueva_categoria_name_url;
			$res[$key][$row->nueva_categoria_id]['img']=$row->categoria_img;
		}

		ksort($res);
		$res_ord = array();
		foreach ($res as $key => $value){
			$zatiak = explode('__', $key, 2);
			$clean_key = isset($zatiak[1]) ? $zatiak[1] : $key;
			$res_ord[$clean_key] = $value;
		}

		return $res_ord;
    }
    function get_categorias_seo_menu_sin_estilos($tipo_producto=-1){
		$a_tipos[0]='Papel Pintado';
		$a_tipos[1]='Murales';
		$a_tipos[2]='Revestimientos';
		$a_tipos[3]='Telas';
		...
		$result=$this->get_categorias_seo($tipo_producto);
		foreach($result as $row){
			if($row->nueva_categoria_activo && $row->idnueva_categoria_familia!=1){
				$res[$a_tipos[$row->tipo_producto].' '.$familias[$row->idnueva_categoria_familia]][$row->nueva_categoria_id]['nombre']=$row->nueva_categoria_name;
				$res[$a_tipos[$row->tipo_producto].' '.$familias[$row->idnueva_categoria_familia]][$row->nueva_categoria_id]['nombre_filtro']=$row->nombre_filtro;
				$res[$a_tipos[$row->tipo_producto].' '.$familias[$row->idnueva_categoria_familia]][$row->nueva_categoria_id]['url']=$row->nueva_categoria_name_url;
				$res[$a_tipos[$row->tipo_producto].' '.$familias[$row->idnueva_categoria_familia]][$row->nueva_categoria_id]['img']=$row->categoria_img;
    ...
    function get_categorias_seo_diferenciadas_menu($tipo_producto=-1){
		$cache_key = 'catdif_' . md5(serialize($tipo_producto));
		$cache_file = APPPATH . 'cache/' . $cache_key . '.cache';
		if (file_exists($cache_file) && (time() - filemtime($cache_file)) < 300) {
			$cached = @unserialize(@file_get_contents($cache_file));
			if ($cached !== false) return $cached;
		...
		$result=$this->get_categorias_seo($tipo_producto);
		foreach($result as $row){
			if($row->nueva_categoria_activo && $row->incluir_desplegable_be==1){
				$res[$a_tipos[$row->tipo_producto]][$row->nueva_categoria_id]['nombre']=$row->nueva_categoria_name;
				$res[$a_tipos[$row->tipo_producto]][$row->nueva_categoria_id]['nombre_filtro']=$row->nombre_filtro;
				$res[$a_tipos[$row->tipo_producto]][$row->nueva_categoria_id]['url']=$row->nueva_categoria_name_url;
			}
		}
    	/*
    ...
    function get_categorias_resaltadas_menu($tipo_producto=-1){
		$res=array();
		$result=$this->get_categorias_seo($tipo_producto);
		foreach($result as $row){
			if($row->nueva_categoria_activo && $row->incluir_desplegable_be==1){
				$res[$row->tipo_producto][$row->nueva_categoria_id]['nombre']=$row->nueva_categoria_name;
				$res[$row->tipo_producto][$row->nueva_categoria_id]['nombre_filtro']=$row->nombre_filtro;
				$res[$row->tipo_producto][$row->nueva_categoria_id]['url']=$row->nueva_categoria_name_url;
			}
		}
    	/*
    ...
    function get_filtros_categoria(){
    	static $cached = null;
    	if ($cached === null)
     		$cached = $this->db->from('nueva_categoria_familia')->order_by('orden_cat_familia','nombre_familia_categoria','ASC','ASC')->get()->result();
    	return $cached;
    }
    ...
    function get_filtros_categoria_array_completo(){
		$result=$this->get_filtros_categoria();
		$res[0]='--';
		foreach($result as $row){
			$res[$row->idnueva_categoria_familia]['nombre']=$row->nombre_familia_categoria;
			$res[$row->idnueva_categoria_familia]['orden']=str_pad($row->orden_cat_familia, 2, '0', STR_PAD_LEFT);
			$res[$row->idnueva_categoria_familia]['txt']=$row->nombre_familia_categoria_txt;
		}
		return $res;
    }
    function get_datos_filtros_categoria(){
		$result=$this->get_filtros_categoria();
		foreach($result as $row){
			$res[$row->idnueva_categoria_familia]=$row;
		}
		return $res;
    }
